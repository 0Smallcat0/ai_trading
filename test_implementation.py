...
def test_strategy_patterns():
    ...
    try:
        ...
        # 測試校驗策略
        time_checksum_strategy = TimeBasedChecksumStrategy(
            ["symbol", "date", "close"], verify_interval_days=7
        )
        critical_checksum_strategy = CriticalDataChecksumStrategy(
            ["symbol", "date", "close"], verify_interval_days=1
        )
        print("✅ 校驗策略創建成功")
