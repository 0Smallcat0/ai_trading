...
class Consumer(ABC):
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # 計算消息消費速率
        messages_per_second = (
            self.stats["messages_consumed"] / uptime if uptime > 0 else 0
        )
...
class WebSocketConsumer(Consumer):
            ...
            try:
                ...
                if self.running:
                    ...
                    if (
                        ...
                        logger.error(
                            f"WebSocket 消費者 '{self.name}' 重連次數超過上限 ({self.max_reconnect})，停止重連"
                        )
                        self.running = False
                        break

                    logger.warning(
                        f"WebSocket 消費者 '{self.name}' 連接關閉，{self.reconnect_interval} 秒後重連 (第 {self.reconnect_count} 次)"
                    )
    ...
    def _on_close(self, ws, close_status_code, close_msg):
        ...
        """
        logger.info(
            f"WebSocket 消費者 '{self.name}' 連接關閉: {close_status_code} {close_msg}"
        )
...
class KafkaConsumer(Consumer):
    ...
    def __init__(
        ...
        **kafka_config,
        ...
        logger.info(
            f"Kafka 消費者 '{name}' 已初始化，服務器: {bootstrap_servers}, 主題: {topics}"
        )
