...
class CrossValidator:
    ...
    def _log_fold_metrics(
        self, fold: int, train_metrics: Dict[str, float], val_metrics: Dict[str, float]
    ) -> None:
        """
        記錄單折指標到 MLflow
    ...
    def _calculate_cv_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 計算平均值和標準差
            avg_train_metrics = self._calculate_metric_statistics(
                all_train_metrics, "mean"
            )
            std_train_metrics = self._calculate_metric_statistics(
                all_train_metrics, "std"
            )
    ...
    def _calculate_metric_statistics(
        self, metrics_list: List[Dict[str, float]], stat_type: str
    ) -> Dict[str, float]:
        """
        計算指標統計值
    ...
    def _find_best_fold(self) -> Dict[str, Any]:
        ...
        best_fold = max(
            self.cv_results, key=lambda x: x["val_metrics"].get(primary_metric, 0)
        )
    ...
    def _log_cv_statistics(self, stats: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if worst_fold:
                ...
                mlflow.log_metric(
                    f"cv_worst_{worst_fold['metric']}", worst_fold["value"]
                )
