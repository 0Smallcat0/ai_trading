...
class DeploymentManager:
    ...
    def deploy_model(
        ...
        try:
            ...
            if environment not in self.environments:
                raise ValueError(
                    f"不支援的環境: {environment}. 支援的環境: {self.environments}"
                )
            ...
            existing_deployment = self.registry.registry["deployments"].get(
                deployment_key
            )
            ...
            deployment_info = {
                ...
                or f"Deploy {model_name} v{version} to {environment}",
    ...
    def list_deployments(
        self, environment: Optional[str] = None, model_name: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        列出部署
        ...
        for deployment_key, deployment_info in self.registry.registry[
            "deployments"
        ].items():
            # 篩選條件
    ...
    def rollback_deployment(
        ...
        try:
            ...
            rollback_deployment.update(
                {
                    "deployed_at": datetime.datetime.now().isoformat(),
                    "description": description
                    or f"Rollback {model_name} in {environment}",
                    "status": "active",
            ...
            logger.info(
                f"部署已回滾: {model_name} {environment} -> v{rollback_deployment['version']}"
            )
    ...
    def undeploy_model(
        ...
        try:
            ...
            # 標記為已取消部署
            self.registry.registry["deployments"][deployment_key][
                "status"
            ] = "undeployed"
            self.registry.registry["deployments"][deployment_key][
                "undeployed_at"
            ...
            ] = (description or "Manual undeploy")
    ...
    def get_deployment_history(
        self, model_name: str, environment: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        獲取部署歷史
