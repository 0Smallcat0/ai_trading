...
class ModelMonitor:
    ...
    def log_prediction(
        ...
        try:
            ...
            # 準備實際值
            actual_value = actual.tolist() if isinstance(actual, np.ndarray) else actual
        ...
        try:
            with mlflow.start_run(run_id=self.model_info["run_id"]):
                ...
                if record["actual"] is not None:
                    ...
                    if isinstance(prediction, (list, np.ndarray)) and isinstance(
    ...
    def calculate_metrics(self) -> Dict[str, float]:
        ...
        try:
            ...
            if self._is_classification_problem():
                metrics.update(
                    self._calculate_classification_metrics(actuals, predictions)
                )
            else:
    ...
    def _calculate_classification_metrics(
        self, actuals: List, predictions: List
    ) -> Dict[str, float]:
        """
        計算分類指標
    ...
    def _calculate_regression_metrics(
        ...
        try:
            ...
            metrics["mape"] = (
                np.mean(np.abs((actuals_array - predictions_array) / actuals_array))
                * 100
            )
