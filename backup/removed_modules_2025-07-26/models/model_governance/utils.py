...
def calculate_model_drift(
    ...
    try:
        ...
        common_features = list(
            set(baseline_features.columns) & set(new_features.columns)
        )
        ...
        # 判斷是否檢測到漂移
        drift_detected = any(
            result.get("drift_detected", False) for result in drift_results.values()
        )
...
def _calculate_ks_drift(
    baseline_data: pd.DataFrame, new_data: pd.DataFrame, threshold: float
...
def _calculate_chi2_drift(
    ...
    for feature in baseline_data.columns:
        try:
            ...
            baseline_counts = baseline_values.value_counts().reindex(
...
def _calculate_psi_drift(
    baseline_data: pd.DataFrame, new_data: pd.DataFrame, threshold: float = 0.1
) -> Dict[str, Dict[str, Any]]:
    """
    ...
    for feature in baseline_data.columns:
        try:
            ...
            # 計算基準和新資料的分佈
            baseline_dist = pd.cut(
                baseline_values, bins=bins, include_lowest=True
            ).value_counts(normalize=True)
            new_dist = pd.cut(new_values, bins=bins, include_lowest=True).value_counts(
                normalize=True
            )

            # 對齊分佈
            baseline_dist = baseline_dist.reindex(
                baseline_dist.index.union(new_dist.index), fill_value=0.001
            )
...
def generate_governance_report(
    registry, model_name: str = None, output_path: str = "governance_report.html"
) -> str:
    """
    生成治理報告
