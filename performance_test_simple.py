...
def test_extended_business_metrics():
    ...
    try:
        # 模擬依賴
        with patch('src.monitoring.business_metrics_extended.Counter'), \
             patch('src.monitoring.business_metrics_extended.Gauge'), \
             patch('src.monitoring.business_metrics_extended.Histogram'):
            
            from src.monitoring.business_metrics_extended import ExtendedBusinessMetricsCollector
            ...
            def update_metrics():
                # 模擬交易數據
                trades = [
                    {"pnl": 100, "entry_time": "2024-01-01T10:00:00", "exit_time": "2024-01-01T11:00:00"},
                    {"pnl": -50, "entry_time": "2024-01-01T11:00:00", "exit_time": "2024-01-01T12:00:00"},
                    {"pnl": 200, "entry_time": "2024-01-01T12:00:00", "exit_time": "2024-01-01T13:00:00"},
                ]
                
                collector.update_trading_performance("test_strategy", "BTCUSDT", trades)
                collector.update_portfolio_metrics("test_strategy", [0.01, -0.005, 0.02])
                collector.update_risk_metrics([0.01, -0.005, 0.02], {"BTCUSDT": 10000}, 100000)
...
def main():
    ...
    print(f"📈 通過率: {total_passed/total_tests*100:.1f}%" if total_tests > 0 else "📈 通過率: 0%")
