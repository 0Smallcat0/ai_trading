...
class TestMissingValueHandler(unittest.TestCase):
    ...
    def test_interpolate(self):
        """測試插值填補"""
        handler = MissingValueHandler(method='interpolate')
    ...
    def test_mean(self):
        """測試均值填補"""
        handler = MissingValueHandler(method='mean')
        result = handler.fit_transform(self.df)
    ...
    def test_median(self):
        """測試中位數填補"""
        handler = MissingValueHandler(method='median')
        result = handler.fit_transform(self.df)
    ...
    def test_ffill(self):
        """測試前向填補"""
        handler = MissingValueHandler(method='ffill')
        result = handler.fit_transform(self.df)
    ...
    def test_bfill(self):
        """測試後向填補"""
        handler = MissingValueHandler(method='bfill')
        result = handler.fit_transform(self.df)
...
class TestOutlierHandler(unittest.TestCase):
    ...
    def test_detect_outliers_zscore(self):
        """測試使用 Z-score 檢測異常值"""
        handler = OutlierHandler(detection_method='z-score', z_threshold=3.0)
    ...
    def test_detect_outliers_iqr(self):
        """測試使用 IQR 檢測異常值"""
        handler = OutlierHandler(detection_method='iqr', iqr_multiplier=1.5)
        outliers = handler.detect_outliers(self.df)
    ...
    def test_treat_outliers_clip(self):
        """測試使用截斷法處理異常值"""
        handler = OutlierHandler(detection_method='z-score', treatment_method='clip')
    ...
    def test_treat_outliers_remove(self):
        """測試使用移除法處理異常值"""
        handler = OutlierHandler(detection_method='z-score', treatment_method='remove')
        result = handler.treat_outliers(self.df)
...
class TestDataCleaningPipeline(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for col in self.df.columns:
            ...
            # 添加異常值
            self.df.loc[mask, col] = self.df.loc[mask, col] + factor * self.df[col].std()
    ...
    def test_clean(self):
        ...
        # 檢查處理步驟是否按順序執行
        self.assertEqual(report['processing_steps'], ['missing_value_handling', 'outlier_handling', 'price_standardization'])
