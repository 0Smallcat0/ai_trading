...
class TestFeatureEngineering(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # 創建目標變數（只與前 5 個特徵相關）
        self.y = pd.Series(
            np.dot(self.X.iloc[:, :5], np.random.randn(5))
        ...
        # 創建測試用的資料字典
        self.data_dict = {"price": self.price_data}
    ...
    def test_dimensionality_reduction_pca(self):
        ...
        self.assertGreater(
            sum(reducer.explained_variance_ratio_), 0.3
        )  # 至少解釋 30% 的方差
    ...
    def test_dimensionality_reduction_pca_auto_components(self):
        ...
        self.assertTrue(hasattr(reducer, "explained_variance_ratio_"))
        self.assertGreater(
            sum(reducer.explained_variance_ratio_), 0.9
        )  # 至少解釋 90% 的方差
    ...
    def test_compute_features_with_selection_and_reduction(self):
        """測試計算特徵並進行選擇和降維"""
        # 由於 compute_features 需要 load_data() 函數，而這個函數可能需要實際的資料庫連接
        # 我們跳過這個測試，或者使用 mock
        try:
            ...
            features = compute_features(
                ...
                clean_data=True,
                feature_selection=False,  # 關閉特徵選擇以避免需要目標變數
                dimensionality_reduction=False,  # 關閉降維以簡化測試
