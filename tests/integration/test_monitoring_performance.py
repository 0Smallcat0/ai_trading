...
class TestMonitoringPerformance:
    ...
    def test_performance_integration(
        ...
        try:
            ...
            # 模擬高負載
            for i in range(100):
                prometheus_collector.record_api_request("GET", f"/api/test{i}", 200, 0.1)
                if i % 10 == 0:
            ...
            assert peak < 100 * 1024 * 1024, f"記憶體使用過多: {peak / 1024 / 1024:.2f}MB"
    ...
    def test_memory_usage_monitoring(
        ...
        try:
            ...
            # 記錄大量指標
            for i in range(1000):
                prometheus_collector.record_api_request("GET", f"/api/memory_test{i}", 200, 0.1)
                ...
                if i % 100 == 0:
                    ...
                    # 記憶體使用不應該無限增長
                    assert current < 50 * 1024 * 1024, f"記憶體使用過多: {current / 1024 / 1024:.2f}MB"
            ...
            assert final_peak < 100 * 1024 * 1024, f"最終記憶體使用過多: {final_peak / 1024 / 1024:.2f}MB"
    ...
    def test_load_testing(
        ...
        try:
            ...
            while time.time() - start_time < test_duration:
                # 模擬各種類型的請求
                prometheus_collector.record_api_request("GET", "/api/load_test", 200, 0.05)
                ...
                prometheus_collector.update_strategy_metrics("load_test_strategy", 1000.0, 1.5, 3.2)
            ...
            # 驗證測試持續時間合理
            assert actual_duration >= test_duration * 0.9, f"測試持續時間過短: {actual_duration:.2f}秒"
            assert actual_duration <= test_duration * 1.5, f"測試持續時間過長: {actual_duration:.2f}秒"
