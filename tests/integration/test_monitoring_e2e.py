...
class TestMonitoringEndToEnd:
    ...
    def test_api_endpoints_integration(
        self, mock_auth: MagicMock, test_client: TestClient
    ) -> None:
        """測試 API 端點整合功能。
        ...
        try:
            ...
            ), f"健康檢查端點失敗: {response.status_code}"
            ...
            assert (
                response.status_code == 200
            ), f"Prometheus 指標端點失敗: {response.status_code}"
            ...
            assert (
                response.status_code == 200
            ), f"告警列表端點失敗: {response.status_code}"
