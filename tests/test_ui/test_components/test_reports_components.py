...
class TestReportGenerator:
    ...
    def test_calculate_trading_metrics(self, sample_trading_data):
        ...
        # 添加必要的欄位
        sample_trading_data['pnl'] = np.random.normal(100, 500, len(sample_trading_data))
        sample_trading_data['trades'] = np.random.randint(1, 10, len(sample_trading_data))
...
class TestRiskAnalyzer:
    ...
    def test_calculate_var(self, sample_trading_data):
        ...
        var_95 = analyzer._calculate_var(sample_trading_data['returns'], confidence=0.95)
        var_99 = analyzer._calculate_var(sample_trading_data['returns'], confidence=0.99)
...
class TestErrorHandling:
    ...
    def test_exception_chaining_in_report_generator(self, sample_trading_data):
        ...
        with patch.object(generator, '_calculate_trading_metrics') as mock_calc:
            ...
            # 驗證異常鏈接
            assert exc_info.value.__cause__ is not None or "計算錯誤" in str(exc_info.value)
...
class TestIntegration:
    ...
    @patch('src.ui.components.reports_components.st')
    def test_full_report_generation_workflow(self, mock_st, sample_trading_data, sample_portfolio_data):
        """測試完整的報表生成工作流程"""
        mock_st.columns.return_value = [Mock(), Mock(), Mock()]
        mock_st.metric = Mock()
        mock_st.plotly_chart = Mock()
    ...
    def test_data_consistency_across_components(self, sample_trading_data):
        ...
        sharpe_ratio = perf_analyzer._calculate_sharpe_ratio(sample_trading_data['returns'])
