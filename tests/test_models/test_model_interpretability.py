...
from src.models.model_interpretability.feature_importance import FeatureImportanceAnalyzer
...
class TestLIMEExplainer:
    ...
    def mock_model(self):
        ...
        model.predict_proba = Mock(return_value=np.array([[0.9, 0.1], [0.2, 0.8], [0.7, 0.3]]))
        return model
...
class TestFeatureImportanceAnalyzer:
    ...
    @patch('src.models.model_interpretability.feature_importance.permutation_importance')
    def test_permutation_importance_calculation(self, mock_perm_importance, mock_model, sample_data):
        """測試排列重要性計算"""
        X, y = sample_data
        ...
        with patch('src.models.model_interpretability.feature_importance.save_explanation_results'):
...
class TestLegacyCompatibility:
    ...
    def test_legacy_interpreter_delegation(self, mock_new_interpreter_class, mock_model):
        """測試舊版解釋器委託"""
        # 模擬新解釋器實例
    ...
    @patch('src.models.model_interpretability.NewModelInterpreter')
    def test_legacy_shap_method_delegation(self, mock_new_interpreter_class, mock_model):
        """測試舊版 SHAP 方法委託"""
        # 模擬新解釋器和結果
        mock_new_interpreter = Mock()
        mock_new_interpreter.explain_with_shap.return_value = {
            'shap_values': np.array([[0.1, 0.2, 0.3, 0.4]]),
            'importance': pd.DataFrame({'feature': ['f1', 'f2'], 'importance': [0.1, 0.2]})
        }
        mock_new_interpreter_class.return_value = mock_new_interpreter
