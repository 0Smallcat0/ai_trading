...
class TestNewModelTrainer:
    ...
    def config(self):
        ...
        return TrainingConfig(
            experiment_name="test_experiment", metrics_threshold={"accuracy": 0.8}
        )
    ...
    def sample_data(self):
        ...
        X_val = pd.DataFrame(
            np.random.randn(30, 5), columns=[f"f{i}" for i in range(5)]
        )
    ...
    def test_train_without_validation(
        self, mock_mlflow, mock_setup, mock_model, config, sample_data
    ):
        """測試不使用驗證集的訓練"""
...
class TestCrossValidator:
    ...
    def test_cross_validate_time_series(
        self, mock_setup, mock_create_model, mock_model, config, sample_data
    ):
        """測試時間序列交叉驗證"""
        X, y = sample_data
        ...
        with patch("src.models.training_pipeline.cross_validator.mlflow"):
            results = validator.cross_validate(
                X, y, cv=3, time_series=True, log_to_mlflow=False
            )
    ...
    def test_cross_validate_kfold(
        ...
        with patch("src.models.training_pipeline.cross_validator.mlflow"):
            results = validator.cross_validate(
                X, y, cv=3, time_series=False, log_to_mlflow=False
            )
    ...
    def test_get_best_model(self, mock_create_model, mock_model, config, sample_data):
        ...
        with patch(
            "src.models.training_pipeline.cross_validator.setup_mlflow_tracking"
