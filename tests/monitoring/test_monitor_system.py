...
class TestMonitorSystem:
    ...
    def mock_monitor_modules(self):
        """模擬監控模組"""
        with patch("src.monitoring.monitor_system.AlertHandler") as mock_alert, patch(
            "src.monitoring.monitor_system.SystemMonitor"
        ) as mock_system, patch(
            "src.monitoring.monitor_system.ThresholdChecker"
        ) as mock_threshold:
    ...
    def mock_external_components(self):
        ...
        with patch(
            ...
            yield {"prometheus": mock_prometheus, "alert_manager": mock_alert_mgr}
    ...
    def test_init_components_prometheus_configure(
        self, mock_monitor_modules, mock_external_components
    ):
        ...
        mock_external_components["prometheus"].configure.assert_called_once_with(
            port=9090, collection_interval=15, api_endpoints=["http://localhost:8000"]
        )

    def test_init_components_alert_manager_configure(
        self, mock_monitor_modules, mock_external_components
    ):
        """測試初始化組件時配置警報管理器"""
        config = {
