...
def test_market_data_tables():
    ...
    with tempfile.NamedTemporaryFile(suffix=".db") as temp_db:
        ...
        with Session(engine) as session:
            ...
            # 檢查查詢
            result_daily = (
                session.query(MarketDaily).filter_by(symbol="2330.TW").first()
            )
            assert result_daily is not None
            assert result_daily.close == 505.0

            result_minute = (
                session.query(MarketMinute).filter_by(symbol="2330.TW").first()
            )
...
def test_data_sharding():
    ...
    with tempfile.NamedTemporaryFile(suffix=".db") as temp_db:
        ...
        with Session(engine) as session:
            ...
            # 檢查查詢
            result = (
                session.query(DataShard).filter_by(table_name="market_daily").first()
            )
