...
class TestRiskManagementEnhanced:
    ...
    def test_risk_scoring_algorithm(self):
        ...
        try:
            ...
            assert high_risk_level in [
                "高風險",
                "極高風險",
            ], f"應該是高風險: {high_risk_level}"
            assert high_risk_color in [
                "🟠",
                "🔴",
            ], f"應該是橙色或紅色: {high_risk_color}"
    ...
    def test_file_structure_enhanced(self):
        """測試增強版文件結構"""
        enhanced_dir = (
            project_root / "src" / "ui" / "pages" / "risk_management_enhanced"
        )
        ...
        # 檢查必要文件存在
        required_files = ["__init__.py", "parameters_enhanced.py", "data_services.py"]
