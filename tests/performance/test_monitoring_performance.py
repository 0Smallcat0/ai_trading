...
class PrometheusCollectorPerformanceTest(PerformanceTestBase):
    ...
    def test_initialization_performance(self):
        ...
        def init_collector():
            with patch("src.monitoring.prometheus_collector.CollectorRegistry"), patch(
                "src.monitoring.prometheus_collector.SystemMetricsCollector"
            ), patch(
                "src.monitoring.prometheus_collector.TradingMetricsCollector"
            ), patch(
                "src.monitoring.prometheus_collector.APIMetricsCollector"
    ...
    def test_metrics_collection_performance(self):
        ...
        concurrent_results = self.run_concurrent_test(
            collect_metrics, num_threads=5, iterations=20
        )
    ...
    def test_dashboard_creation_performance(self):
        ...
        def create_dashboard():
            ...
            return self.manager.create_custom_dashboard(
                "Performance Test Dashboard", ["test", "performance"], panel_configs
            )
...
class MonitorSystemPerformanceTest(PerformanceTestBase):
    ...
    def test_status_check_performance(self):
        ...
        # 測量並發狀態檢查
        concurrent_results = self.run_concurrent_test(
            check_status, num_threads=10, iterations=50
        )
                ...
                try:
                    ...
                    if "time_ms" in str(result):
                        for key, value in result.items():
                            ...
                            elif "throughput" in key and isinstance(
                                value, (int, float)
                            ):
