...
def test_portfolio_module_exists():
    ...
    module_path = os.path.join(os.path.dirname(__file__), '..', 'src', 'core', 'portfolio.py')
...
def test_portfolio_module_content():
    ...
    assert 'def compare_portfolio_performance(' in content, "compare_portfolio_performance 函數不存在"
...
def test_portfolio_methods():
    ...
    assert 'def _update_positions_value(' in content, "Portfolio._update_positions_value 方法不存在"
    assert 'def _record_state(' in content, "Portfolio._record_state 方法不存在"
    assert 'def _execute_rebalance(' in content, "Portfolio._execute_rebalance 方法不存在"
    assert 'def _buy_stock(' in content, "Portfolio._buy_stock 方法不存在"
    assert 'def _sell_stock(' in content, "Portfolio._sell_stock 方法不存在"
    assert 'def _calculate_performance(' in content, "Portfolio._calculate_performance 方法不存在"
    ...
    assert 'def plot_portfolio_performance(' in content, "plot_portfolio_performance 函數不存在"
