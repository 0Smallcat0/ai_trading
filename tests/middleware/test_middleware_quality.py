...
class TestMiddlewareImports:
    ...
    def test_auth_middleware_import(self):
        """測試認證中間件導入"""
        try:
            from src.api.middleware.auth import AuthMiddleware, TokenManager, PermissionChecker
    ...
    def test_rate_limit_middleware_import(self):
        """測試速率限制中間件導入"""
        try:
            from src.api.middleware.rate_limit import RateLimitMiddleware, RateLimitRule, TokenBucket
...
class TestMiddlewareBasicFunctionality:
    ...
    def test_rate_limit_rule(self):
        ...
        rule = RateLimitRule(max_requests=100, window_seconds=60, description="測試規則")
