...
def demo_backpressure_control():
    ...
    for i in range(60):
        ...
        if i % 10 == 0:
            ...
            if stats:
                logger.info(f"背壓統計: 當前間隔={stats['current_interval']:.3f}s, "
    ...
    for i in range(10):
        ...
        if best_source:
            try:
                ...
                # 記錄成功
                failover_manager.record_request_result(
                    best_source, True, response_time
                )
                ...
                # 記錄失敗
                failover_manager.record_request_result(
                    best_source, False, 0.0, str(e)
                )
                logger.error(f"請求失敗: {e}")
        
        # 顯示健康狀態
        summary = failover_manager.get_health_summary()
        logger.info(f"健康狀態: 健康={summary['healthy_sources']}, "
