...
class DeploymentConfigValidator:
        ...
        try:
            ...
            if "services" in compose_data:
                for service_name, service_config in compose_data["services"].items():
                    service_result = self._validate_service(service_name, service_config, file_path.name)
    ...
    def _validate_service(self, service_name: str, config: Dict, filename: str) -> Dict[str, Any]:
        """驗證服務配置"""
                    ...
                    if isinstance(env_var, str) and "=" in env_var:
                        ...
                        if any(secret in key.upper() for secret in ["PASSWORD", "SECRET", "KEY", "TOKEN"]):
                            if not value.startswith("${"):
                                result["issues"].append(f"敏感資訊 {key} 建議使用環境變數")
    ...
    def _validate_k8s_yaml(self, file_path: Path) -> Dict[str, Any]:
        ...
        result = {"file": str(file_path.relative_to(self.project_root)), "valid": True, "issues": []}
    ...
    def _validate_k8s_deployment(self, deployment: Dict, result: Dict):
        ...
        for container in containers:
            if "resources" not in container:
                result["issues"].append(f"容器 {container.get('name', 'unknown')} 建議設定資源限制")
            if "livenessProbe" not in container:
                result["issues"].append(f"容器 {container.get('name', 'unknown')} 建議設定存活探針")
            if "readinessProbe" not in container:
                result["issues"].append(f"容器 {container.get('name', 'unknown')} 建議設定就緒探針")
                ...
                if line and not line.startswith('#') and '=' in line:
                    ...
                    if any(secret in key.upper() for secret in ["PASSWORD", "SECRET", "KEY", "TOKEN"]):
                        if value in ["changeme", "password", "secret", "your_key_here"]:
                            result["issues"].append(f"第 {line_num} 行: {key} 包含範例值")
