...
class PerformanceRegressionTester:
    ...
    def __init__(self, project_root: str = "."):
        ...
        self.comparison_file = (
            self.reports_dir / CI_PERFORMANCE_CONFIG["comparison_file"]
        )
    ...
    def compare_performance(
        ...
        current_benchmarks = {
            bench["name"]: bench for bench in current.get("benchmarks", [])
        }

        # 比較共同的基準測試
        common_benchmarks = set(baseline_benchmarks.keys()) & set(
            current_benchmarks.keys()
        )
...
def main():
    ...
    parser.add_argument("--update-baseline", action="store_true", help="更新效能基準")
    parser.add_argument(
        "--no-fail-on-regression", action="store_true", help="即使有回歸也不失敗"
    )
