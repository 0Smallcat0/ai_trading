...
class StressTester:
    ...
    def __init__(
        ...
        self.output_dir = output_dir or os.path.join(
            RESULTS_DIR, "stress_test", datetime.now().strftime("%Y%m%d%H%M%S")
        )
    ...
    def simulate_market_crash(
        ...
        # 模擬崩盤
        crash_start, crash_end = pd.to_datetime(crash_period[0]), pd.to_datetime(
            crash_period[1]
        )
        crash_mask = (stress_data["date"] >= crash_start) & (
            stress_data["date"] <= crash_end
        )
        ...
        for col in ["open", "high", "low", "close"]:
            if col in stress_data.columns:
                # 獲取崩盤前的最後一個價格
                last_price_before_crash = stress_data.loc[
                    ~crash_mask & (stress_data["date"] < crash_start), col
                ].iloc[-1]
        ...
        if recovery_period is not None and recovery_percentage is not None:
            recovery_start, recovery_end = pd.to_datetime(
                recovery_period[0]
            ), pd.to_datetime(recovery_period[1])
            recovery_mask = (stress_data["date"] >= recovery_start) & (
                stress_data["date"] <= recovery_end
            )
            ...
            for col in ["open", "high", "low", "close"]:
                if col in stress_data.columns:
                    # 獲取復甦前的最後一個價格
                    last_price_before_recovery = stress_data.loc[
                        ~recovery_mask & (stress_data["date"] < recovery_start), col
                    ].iloc[-1]
        ...
        # 保存壓力測試資料
        stress_data.to_csv(
            os.path.join(self.output_dir, "market_crash_data.csv"), index=False
        )
    ...
    def simulate_liquidity_crisis(
        ...
        # 模擬流動性危機
        crisis_start, crisis_end = pd.to_datetime(crisis_period[0]), pd.to_datetime(
            crisis_period[1]
        )
        crisis_mask = (stress_data["date"] >= crisis_start) & (
            stress_data["date"] <= crisis_end
        )

        # 減少成交量
        if "volume" in stress_data.columns:
            stress_data.loc[crisis_mask, "volume"] = stress_data.loc[
                crisis_mask, "volume"
            ] * (1 - volume_reduction)
        ...
        if "high" in stress_data.columns and "low" in stress_data.columns:
            ...
            stress_data.loc[crisis_mask, "high"] = (
                stress_data.loc[crisis_mask, "high"] + spread_diff[crisis_mask]
            )
            stress_data.loc[crisis_mask, "low"] = (
                stress_data.loc[crisis_mask, "low"] - spread_diff[crisis_mask]
            )

        # 執行壓力測試（需要在回測函數中處理滑價增加）
        stress_result = self.backtest_func(
            stress_data, slippage_multiplier=slippage_increase
        )

        # 保存壓力測試資料
        stress_data.to_csv(
            os.path.join(self.output_dir, "liquidity_crisis_data.csv"), index=False
        )
    ...
    def simulate_volatility_shock(
        self, shock_period: Tuple[str, str], volatility_multiplier: float = 3.0
    ) -> Dict[str, Any]:
        """
        模擬波動率衝擊
        ...
        # 模擬波動率衝擊
        shock_start, shock_end = pd.to_datetime(shock_period[0]), pd.to_datetime(
            shock_period[1]
        )
        shock_mask = (stress_data["date"] >= shock_start) & (
            stress_data["date"] <= shock_end
        )
        ...
        if "close" in stress_data.columns:
            ...
            new_returns = np.random.normal(
                mean_return, new_std_return, len(shock_returns)
            )
            ...
            # 重新計算價格
            last_price_before_shock = stress_data.loc[
                ~shock_mask & (stress_data["date"] < shock_start), "close"
            ].iloc[-1]
            ...
            # 調整 open, high, low
            if (
                "open" in stress_data.columns
                and "high" in stress_data.columns
                and "low" in stress_data.columns
            ):
                # 計算與收盤價的比例
                open_ratio = stress_data.loc[shock_mask, "open"] / stress_data.loc[
                    shock_mask, "close"
                ].shift(1)
                high_ratio = (
                    stress_data.loc[shock_mask, "high"]
                    / stress_data.loc[shock_mask, "close"]
                )
                low_ratio = (
                    stress_data.loc[shock_mask, "low"]
                    / stress_data.loc[shock_mask, "close"]
                )

                # 應用比例計算新價格
                stress_data.loc[shock_mask, "open"] = (
                    stress_data.loc[shock_mask, "close"].shift(1) * open_ratio
                )
                stress_data.loc[shock_mask, "high"] = (
                    stress_data.loc[shock_mask, "close"] * high_ratio
                )
                stress_data.loc[shock_mask, "low"] = (
                    stress_data.loc[shock_mask, "close"] * low_ratio
                )
        ...
        # 保存壓力測試資料
        stress_data.to_csv(
            os.path.join(self.output_dir, "volatility_shock_data.csv"), index=False
        )
    ...
    def simulate_correlation_change(
        self, assets: List[str], change_period: Tuple[str, str], new_correlation: float
    ) -> Dict[str, Any]:
        """
        模擬相關性變化
        ...
        # 模擬相關性變化
        change_start, change_end = pd.to_datetime(change_period[0]), pd.to_datetime(
            change_period[1]
        )
        change_mask = (stress_data["date"] >= change_start) & (
            stress_data["date"] <= change_end
        )
        ...
        for i, asset in enumerate(assets):
            # 獲取變化前的最後一個價格
            last_price_before_change = asset_prices[asset][
                ~change_mask & (stress_data["date"] < change_start)
            ].iloc[-1]
            ...
            if f"{asset}_close" in stress_data.columns:
                ...
                stress_data.loc[
                    change_mask & (stress_data["symbol"] == asset), "close"
                ] = new_prices.values
        ...
        # 保存壓力測試資料
        stress_data.to_csv(
            os.path.join(self.output_dir, "correlation_change_data.csv"), index=False
        )
    ...
    def _plot_volatility_comparison(
        ...
        if "close" in original_data.columns:
            ...
            original_volatility = original_returns.rolling(
                window=window
            ).std() * np.sqrt(252)
        ...
        if "close" in stress_data.columns:
            ...
            stress_volatility = stress_returns.rolling(window=window).std() * np.sqrt(
                252
            )
    ...
    def _plot_correlation_comparison(
        ...
        # 獲取原始資料中的變化期間
        original_change_mask = (original_data["date"] >= change_start) & (
            original_data["date"] <= change_end
        )
        original_change_data = original_data[original_change_mask]

        # 獲取壓力測試資料中的變化期間
        stress_change_mask = (stress_data["date"] >= change_start) & (
            stress_data["date"] <= change_end
        )
        ...
        for asset in assets:
            if f"{asset}_close" in original_change_data.columns:
                original_returns[asset] = (
                    original_change_data[f"{asset}_close"].pct_change().dropna()
                )
            elif (
                "symbol" in original_change_data.columns
                and "close" in original_change_data.columns
            ):
                asset_data = original_change_data[
                    original_change_data["symbol"] == asset
                ]
        ...
        for asset in assets:
            if f"{asset}_close" in stress_change_data.columns:
                stress_returns[asset] = (
                    stress_change_data[f"{asset}_close"].pct_change().dropna()
                )
            elif (
                "symbol" in stress_change_data.columns
                and "close" in stress_change_data.columns
            ):
        ...
        # 繪製原始相關係數熱圖
        sns.heatmap(
            original_corr, annot=True, cmap="coolwarm", vmin=-1, vmax=1, ax=axes[0]
        )
        axes[0].set_title("原始相關係數")

        # 繪製壓力測試相關係數熱圖
        sns.heatmap(
            stress_corr, annot=True, cmap="coolwarm", vmin=-1, vmax=1, ax=axes[1]
        )
