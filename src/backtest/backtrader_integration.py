...
class ModelSignalStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        """
        # 初始化指標
        self.atr = bt.indicators.ATR(self.data, period=self.p.atr_period) if self.p.use_atr_stop else None
    ...
    def next(self):
        ...
        if self.last_rebalance_date is None or (current_date - self.last_rebalance_date).days >= self.p.rebalance_freq:
            ...
            for data in self.datas:
                ...
                if signal > self.p.signal_threshold and not position:
                    ...
                    if self.get_position_count() < self.p.max_positions:
                        ...
                        # 買入
                        self.log(f'買入訊號: {data._name}, 訊號值={signal:.2f}, 倉位大小={size}')
                        self.order = self.buy(data=data, size=size)
                elif signal < -self.p.signal_threshold and position:
                    # 賣出
                    self.log(f'賣出訊號: {data._name}, 訊號值={signal:.2f}, 倉位大小={position.size}')
...
class BacktestEngine:
    ...
    def __init__(
        ...
        self.tax = tax
        self.output_dir = output_dir or os.path.join(RESULTS_DIR, "backtest", datetime.now().strftime("%Y%m%d%H%M%S"))
    ...
    def run(self) -> Dict[str, Any]:
        ...
        days = (self.end_date - self.start_date).days if self.end_date and self.start_date else 365
