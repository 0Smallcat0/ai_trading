...
class MLStrategy(ModelSignalStrategy):
    ...
    def prepare_features(self):
        ...
        feature_columns = self.p.feature_columns or ['open', 'high', 'low', 'close', 'volume']
            ...
            for i in range(min(self.p.feature_window, len(data))):
                for col in feature_columns:
                    if hasattr(data.lines, col):
                        features[f'{data._name}_{col}_{i}'] = getattr(data.lines, col)[-i]
    ...
    def next(self):
        ...
        if self.p.model is not None and hasattr(self.p.model, 'train') and (self.last_train_date is None or (current_date - self.last_train_date).days >= self.p.retrain_freq):
            ...
            # 重新訓練模型
            if len(self.feature_history) >= self.p.retrain_window and len(self.target_history) >= self.p.retrain_window:
                self.retrain_model()
    ...
    def collect_training_data(self):
        ...
        for data in self.datas:
            # 計算未來收益率
            future_price = data.close[self.p.prediction_window] if len(data) > self.p.prediction_window else data.close[0]
            current_price = data.close[0]
            future_return = (future_price / current_price) - 1
            
            # 根據未來收益率生成目標
            if self.p.model is not None and hasattr(self.p.model, 'is_classifier') and self.p.model.is_classifier:
                # 分類目標
...
class EnsembleStrategy(ModelSignalStrategy):
    ...
    params = (
        ('models', []),  # 模型列表
        ('weights', []),  # 權重列表
        ('voting_method', 'weighted'),  # 投票方法，可選 'weighted', 'majority', 'average'
    )
    ...
    def generate_signals(self):
        ...
        if self.p.voting_method == 'weighted':
            # 加權平均
            for data in self.datas:
                signals[data._name] = sum(model_signal[data._name] * weight for model_signal, weight in zip(model_signals, self.p.weights))
        elif self.p.voting_method == 'majority':
            # 多數投票
            for data in self.datas:
                votes = [1 if model_signal[data._name] > self.p.signal_threshold else 0 for model_signal in model_signals]
                signals[data._name] = 1.0 if sum(votes) > len(votes) / 2 else 0.0
        elif self.p.voting_method == 'average':
            # 平均
            for data in self.datas:
                signals[data._name] = sum(model_signal[data._name] for model_signal in model_signals) / len(model_signals)
        else:
...
class TechnicalStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        for data in self.datas:
            ...
            # MACD
            macd = btind.MACD(data, period_me1=self.p.macd_fast, period_me2=self.p.macd_slow, period_signal=self.p.macd_signal)
            self.macd[data._name] = macd.macd
            self.macd_signal[data._name] = macd.signal
            self.macd_hist[data._name] = macd.macd - macd.signal
            
            # 布林通道
            bb = btind.BollingerBands(data, period=self.p.bb_period, devfactor=self.p.bb_dev)
        ...
        if order.status in [order.Completed]:
            ...
            if order.isbuy():
                ...
                if self.p.stop_loss > 0:
                    ...
                    stop_order = self.sell(exectype=bt.Order.Stop, price=stop_price, size=order.executed.size)
                    self.stop_orders[order.data._name] = stop_order
                
                if self.p.take_profit > 0:
                    take_profit_price = order.executed.price * (1.0 + self.p.take_profit)
                    take_profit_order = self.sell(exectype=bt.Order.Limit, price=take_profit_price, size=order.executed.size)
        ...
        for data in self.datas:
            ...
            if self.p.trailing_stop and position:
                ...
                if data._name in self.stop_orders:
                    ...
                    new_stop_price = current_price * (1.0 - self.p.trailing_stop_distance)
                    
                    # 如果新的止損價格高於當前止損價格，則更新止損價格
                    if new_stop_price > stop_order.price:
                        self.cancel(stop_order)
                        self.stop_orders[data._name] = self.sell(data=data, exectype=bt.Order.Stop, price=new_stop_price, size=position.size)
                        self.log(f'更新追蹤止損: {data._name}, 新止損價格={new_stop_price:.2f}')
