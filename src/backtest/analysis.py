...
class BacktestAnalyzer:
    ...
    def __init__(
        ...
        self.output_dir = output_dir or os.path.join(RESULTS_DIR, "backtest_analysis", datetime.now().strftime("%Y%m%d%H%M%S"))
    ...
    def _calculate_metrics(self) -> Dict[str, float]:
        ...
        if self.equity_curve is not None and "equity" in self.equity_curve.columns:
            ...
            # 計算累積收益率
            self.equity_curve["cum_return"] = (1 + self.equity_curve["return"]).cumprod() - 1
        ...
        if self.benchmark is not None and "close" in self.benchmark.columns:
            ...
            if self.equity_curve is not None and "return" in self.equity_curve.columns:
                # 確保日期對齊
                if "date" in self.equity_curve.columns and "date" in self.benchmark.columns:
                    # 合併資料
                    merged = pd.merge(
                    ...
                    # 計算資訊比率
                    metrics["information_ratio"] = merged["excess_return"].mean() / merged["excess_return"].std() * np.sqrt(252)
                    
                    # 計算 Beta
                    metrics["beta"] = np.cov(merged["return_strategy"], merged["return_benchmark"])[0, 1] / np.var(merged["return_benchmark"])
                    
                    # 計算 Alpha
                    metrics["alpha"] = metrics["annual_return"] - 0.02 - metrics["beta"] * (self.benchmark["return"].mean() * 252 - 0.02)
                    
                    # 計算相關係數
                    metrics["correlation"] = np.corrcoef(merged["return_strategy"], merged["return_benchmark"])[0, 1]
                    ...
                    if len(up_market) > 0:
                        metrics["up_capture"] = up_market["return_strategy"].mean() / up_market["return_benchmark"].mean()
                    else:
                        metrics["up_capture"] = 0.0
                    
                    if len(down_market) > 0:
                        metrics["down_capture"] = down_market["return_strategy"].mean() / down_market["return_benchmark"].mean()
                    else:
    ...
    def plot_equity_curve(self, filename: Optional[str] = None) -> str:
        ...
        # 繪製基準指數
        if self.benchmark is not None and "close" in self.benchmark.columns and "date" in self.benchmark.columns:
            # 將基準指數標準化為與策略起始資金相同
            benchmark_equity = self.benchmark["close"] / self.benchmark["close"].iloc[0] * self.metrics["initial_cash"]
            plt.plot(self.benchmark["date"], benchmark_equity, label="基準指數", alpha=0.7)
    ...
    def plot_drawdown(self, filename: Optional[str] = None) -> str:
        ...
        plt.fill_between(self.equity_curve["date"], self.equity_curve["drawdown"] * 100, 0, alpha=0.3, color="red")
    ...
    def plot_monthly_returns(self, filename: Optional[str] = None) -> str:
        ...
        # 計算每月最後一天的權益
        monthly_equity = self.equity_curve.groupby(["year", "month"])["equity"].last().reset_index()
    ...
    def perform_sensitivity_analysis(
        ...
        # 設定檔案名稱
        if filename is None:
            filename = os.path.join(self.output_dir, f"sensitivity_{parameter_name}.png")
        ...
        plt.plot(sensitivity_df["parameter_value"], sensitivity_df["metric_value"], marker="o")
    ...
    def perform_robustness_analysis(
        ...
        plt.axvline(median, color="green", linestyle="-.", label=f"中位數: {median:.4f}")
        plt.axvline(mean - std, color="orange", linestyle=":", label=f"平均值 - 標準差: {mean - std:.4f}")
        plt.axvline(mean + std, color="orange", linestyle=":", label=f"平均值 + 標準差: {mean + std:.4f}")
