...
class ConfigValidator:
    ...
    def add_required_config(self, name: str, severity: ConfigSeverity = ConfigSeverity.ERROR, description: Optional[str] = None):
        """添加必要配置項"""
    ...
    def validate_config_dict(self, config: Dict[str, Any]) -> ConfigValidationResult:
        ...
        for name, severity, description in self.required_configs:
            if name not in config or config[name] is None or (isinstance(config[name], str) and config[name].strip() == ""):
...
def create_trading_validator() -> ConfigValidator:
    ...
    # 交易設定
    validator.add_required_config("TRADING_HOURS_START", ConfigSeverity.WARNING, "交易開始時間")
    validator.add_required_config("TRADING_HOURS_END", ConfigSeverity.WARNING, "交易結束時間")
    validator.add_required_config("MAX_POSITION_SIZE", ConfigSeverity.WARNING, "最大持倉比例")
    validator.add_required_config("STOP_LOSS_THRESHOLD", ConfigSeverity.WARNING, "停損閾值")
...
def create_monitoring_validator() -> ConfigValidator:
    ...
    # 監控設定
    validator.add_required_config("PRICE_ANOMALY_THRESHOLD", ConfigSeverity.WARNING, "價格異常閾值")
    validator.add_required_config("VOLUME_ANOMALY_THRESHOLD", ConfigSeverity.WARNING, "成交量異常閾值")
    ...
    validator.add_required_config("GRAFANA_ADMIN_USER", ConfigSeverity.INFO, "Grafana 管理員用戶名")
    validator.add_required_config("GRAFANA_ADMIN_PASSWORD", ConfigSeverity.INFO, "Grafana 管理員密碼")
