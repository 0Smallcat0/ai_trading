...
def _show_fallback_portfolio_management() -> None:
    ...
    **投資組合管理系統** 提供完整的組合管理功能，包括：
    ...
    for allocation in allocation_data:
        ...
        with col4:
            deviation_color = "🔴" if allocation['偏差'].startswith('+') else "🟢" if allocation['偏差'].startswith('-') else "⚪"
    ...
    holdings = [
        {"股票": "AAPL", "數量": "100", "市值": "$15,000", "權重": "12%", "收益": "+8.5%"},
        {"股票": "GOOGL", "數量": "50", "市值": "$12,500", "權重": "10%", "收益": "+12.3%"},
        {"股票": "MSFT", "數量": "75", "市值": "$22,500", "權重": "18%", "收益": "+6.8%"},
        {"股票": "TSLA", "數量": "25", "市值": "$5,000", "權重": "4%", "收益": "-2.1%"}
    ]
    ...
    for holding in holdings:
        with st.expander(f"{holding['股票']} - 權重: {holding['權重']}"):
            ...
            with col3:
                if st.button(f"調整", key=f"adjust_{holding['股票']}"):
    ...
    for suggestion in rebalance_suggestions:
        action_color = "🔴" if suggestion["操作"] == "減持" else "🟢" if suggestion["操作"] == "增持" else "🟡"
        st.markdown(f"{action_color} **{suggestion['操作']} {suggestion['股票']}**: {suggestion['建議']} - {suggestion['原因']}")
...
def _show_fallback_text_analysis() -> None:
    ...
    **文本分析系統** 提供市場情緒和文本分析功能，包括：
    ...
    for signal in signals:
        signal_color = "🟢" if signal["信號"] == "買入" else "🟡" if signal["信號"] == "觀望" else "🔴"
        st.markdown(f"{signal_color} **{signal['信號']} {signal['股票']}** (強度: {signal['強度']}) - {signal['原因']}")
