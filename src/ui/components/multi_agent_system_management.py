...
def _show_fallback_multi_agent_dashboard() -> None:
    ...
    **多代理系統管理** 提供完整的多代理協調功能，包括：
    ...
    agents = [
        {"名稱": "數據分析代理", "類型": "分析型", "狀態": "🟢 運行中", "效率": "92%", "任務數": "15"},
        {"名稱": "策略執行代理", "類型": "執行型", "狀態": "🟢 運行中", "效率": "88%", "任務數": "8"},
        {"名稱": "風險監控代理", "類型": "監控型", "狀態": "🟢 運行中", "效率": "95%", "任務數": "12"},
        {"名稱": "決策支援代理", "類型": "決策型", "狀態": "🟡 待機", "效率": "85%", "任務數": "3"},
        {"名稱": "學習優化代理", "類型": "學習型", "狀態": "🔴 離線", "效率": "78%", "任務數": "0"}
    ]
    ...
    for agent in agents:
        with st.expander(f"{agent['名稱']} - {agent['狀態']}"):
            ...
            with col3:
                if st.button(f"管理", key=f"manage_{agent['名稱']}"):
...
def _show_fallback_advanced_monitoring() -> None:
    ...
    **高級監控系統** 提供深度監控和分析功能，包括：
    ...
    chart_type = st.selectbox("選擇圖表類型", ["性能趨勢", "錯誤分析", "資源使用", "代理效率"])
    ...
    anomalies = [
        {"時間": "14:30", "類型": "性能異常", "嚴重性": "🟡 中等", "描述": "響應時間異常升高"},
        {"時間": "13:45", "類型": "資源異常", "嚴重性": "🟢 低", "描述": "記憶體使用略高"},
        {"時間": "12:20", "類型": "代理異常", "嚴重性": "🔴 高", "描述": "決策代理響應超時"}
    ]
    
    for anomaly in anomalies:
        st.markdown(f"**{anomaly['時間']}** {anomaly['嚴重性']} {anomaly['類型']} - {anomaly['描述']}")
