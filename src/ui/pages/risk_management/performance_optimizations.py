...
def calculate_complex_risk_metrics(portfolio_data: Dict[str, Any]) -> Dict[str, float]:
    ...
    for scenario, shock in stress_scenarios.items():
        stress_results[f"stress_{scenario}"] = (
            portfolio_data.get("value", 1000000) * shock
        )
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if duration > 2.0:
                    st.warning(
                        f"⚠️ 操作 '{operation_name}' 執行時間較長: {duration:.2f}s"
                    )
...
def get_performance_metrics() -> Dict[str, Any]:
    ...
    return {
        "cache_size": len(
            [k for k in st.session_state.keys() if k.startswith("cache_")]
        ),
