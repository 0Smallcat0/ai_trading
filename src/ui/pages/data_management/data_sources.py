...
try:
    ...
    # 提供簡化的替代函數
    def show_data_source_status_card(source_name: str, source_info: Dict[str, Any]) -> None:
        """簡化的資料來源狀態卡片"""
        with st.container():
    ...
    def show_connection_test_result(source_name: str, is_connected: bool, message: str) -> None:
        """簡化的連接測試結果顯示"""
        if is_connected:
...
def show_data_sources_overview() -> None:
    ...
    with col4:
        st.metric(
            "離線", 
            status_counts.get("離線", 0) + status_counts.get("錯誤", 0)
        )
