...
def show_export_configuration() -> Dict[str, Any]:
    ...
    return {
        ...
        'symbols': symbols,
        'start_date': date_range[0] if len(date_range) > 0 else datetime.now() - timedelta(days=30),
    ...
    if st.button("🔍 預覽資料"):
        with st.spinner("正在載入資料預覽..."):
            ...
            if preview_data is not None and not preview_data.empty:
                ...
                with col3:
                    st.metric("資料大小", f"{preview_data.memory_usage(deep=True).sum() / 1024:.1f} KB")
            ...
            if export_data is not None and not export_data.empty:
                ...
                if file_content is not None:
                    ...
                    st.download_button(
                        ...
                        data=file_content,
                        file_name=filename,
                        mime=mime_types.get(config['export_format'], 'application/octet-stream')
                    )
                else:
                    st.error("❌ 匯出失敗")
            else:
