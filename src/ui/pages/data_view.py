...
INDICATOR_INFO = {
    "RSI": {
        ...
        "description": "RSI是一個動量震盪指標，用於衡量價格變動的速度和變化幅度。",
        "calculation": "RSI = 100 - (100 / (1 + RS))，其中RS = 平均上漲幅度 / 平均下跌幅度",
        "range": "0-100",
        "signals": {
            "超買": "RSI > 70，可能出現賣出信號",
            "超賣": "RSI < 30，可能出現買入信號",
            "背離": "價格創新高而RSI未創新高，可能反轉"
        },
        "application": "適用於識別超買超賣狀態，尋找反轉機會。在趨勢市場中效果較好。",
        "period": "通常使用14天週期"
    },
    "MACD": {
        "name": "移動平均收斂散度 (Moving Average Convergence Divergence)",
        "description": "MACD是一個趨勢跟隨動量指標，顯示兩個移動平均線之間的關係。",
        "calculation": "MACD線 = EMA(12) - EMA(26)，信號線 = EMA(MACD, 9)，柱狀圖 = MACD - 信號線",
        "range": "無限制範圍",
        "signals": {
            "金叉": "MACD線上穿信號線，買入信號",
            "死叉": "MACD線下穿信號線，賣出信號",
        ...
        "application": "適用於趨勢確認和動量分析。在趨勢明確的市場中表現優異。",
        "period": "標準參數：快線12，慢線26，信號線9"
    },
    "Williams %R": {
        "name": "威廉指標 (Williams %R)",
        "description": "威廉指標是一個動量指標，衡量收盤價在過去N天最高價和最低價範圍內的位置。",
        "calculation": "%R = (最高價 - 收盤價) / (最高價 - 最低價) × (-100)",
        "range": "-100 到 0",
        "signals": {
            "超買": "%R > -20，可能出現賣出信號",
            "超賣": "%R < -80，可能出現買入信號",
        ...
        "application": "適用於短期交易，識別超買超賣狀態。反應靈敏，適合快速決策。",
        "period": "通常使用14天週期"
    },
    "Stochastic": {
        "name": "隨機指標 (Stochastic Oscillator)",
        "description": "隨機指標比較收盤價與一定時期內價格範圍的關係，包含%K和%D兩條線。",
        "calculation": "%K = (收盤價 - 最低價) / (最高價 - 最低價) × 100，%D = %K的移動平均",
        "range": "0-100",
        "signals": {
            "超買": "%K或%D > 80，可能出現賣出信號",
            "超賣": "%K或%D < 20，可能出現買入信號",
        ...
        "application": "適用於震盪市場，識別轉折點。與RSI類似但更敏感。",
        "period": "通常使用%K(14,3)，%D(3)"
    },
    "CCI": {
        "name": "商品通道指數 (Commodity Channel Index)",
        "description": "CCI衡量價格偏離其統計平均值的程度，用於識別週期性轉折點。",
        "calculation": "CCI = (典型價格 - 移動平均) / (0.015 × 平均絕對偏差)",
        "range": "通常在-100到+100之間",
        "signals": {
            "超買": "CCI > +100，可能出現賣出信號",
            "超賣": "CCI < -100，可能出現買入信號",
            "零軸突破": "穿越零軸確認趨勢"
        },
        "application": "適用於識別價格極端狀態和趨勢轉折。在商品市場表現優異。",
        "period": "通常使用20天週期"
    },
    "ATR": {
        "name": "平均真實範圍 (Average True Range)",
        "description": "ATR衡量價格波動的幅度，用於評估市場的波動性。",
        "calculation": "TR = max(高-低, |高-前收|, |低-前收|)，ATR = TR的移動平均",
        "range": "大於0的數值",
        "signals": {
            "高波動": "ATR值較高，市場波動劇烈",
            "低波動": "ATR值較低，市場相對平靜",
        ...
        "application": "適用於設定止損位、評估市場波動風險和確認趨勢強度。不提供方向信號。",
        "period": "通常使用14天週期"
    }
}

def show_indicator_info(indicator_key: str):
    """顯示指標詳細信息的懸停提示"""
    if indicator_key not in INDICATOR_INFO:
        return
    ...
    # 創建詳細信息的HTML內容
    info_html = f"""
    <div style="background-color: #f0f2f6; padding: 15px; border-radius: 10px; border-left: 4px solid #1f77b4; margin: 10px 0;">
        <h4 style="color: #1f77b4; margin-top: 0;">{info['name']}</h4>

        <p><strong>📝 描述：</strong><br>{info['description']}</p>

        <p><strong>🧮 計算公式：</strong><br>{info['calculation']}</p>

        <p><strong>📊 數值範圍：</strong> {info['range']}</p>

        <p><strong>📈 交易信號：</strong></p>
        <ul>
    """

    for signal, description in info['signals'].items():
        info_html += f"<li><strong>{signal}：</strong>{description}</li>"

    info_html += f"""
        </ul>

        <p><strong>💡 應用建議：</strong><br>{info['application']}</p>

        <p><strong>⏱️ 週期參數：</strong> {info['period']}</p>
    </div>
    """
    ...
    with col2:
        ...
        with st.expander("ℹ️", expanded=False):
            if indicator_key in INDICATOR_INFO:
                ...
                st.markdown(f"📊 **範圍：** {info['range']}")
                st.markdown(f"⏱️ **週期：** {info['period']}")

                st.markdown("**📈 交易信號：**")
                for signal, description in info['signals'].items():
                    st.markdown(f"• **{signal}：** {description}")

                st.markdown(f"**💡 應用：** {info['application']}")
            else:
                st.markdown("暫無詳細信息")
