...
class MovingAverageCrossStrategy(Strategy):
    ...
    def _validate_parameters(self) -> None:
        ...
        if not isinstance(self.long_window, int) or self.long_window <= 0:
            raise ParameterError(
                f"long_window 必須是正整數，得到: {self.long_window}"
            )
            ...
            for long_window in param_grid["long_window"]:
                ...
                try:
                    ...
                    logger.warning(
                        ...
                        short_window, long_window, str(e)
                    )
                    continue
    ...
    def get_signal_summary(self, signals: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_holding_period": float(holding_periods / buy_signals) if buy_signals > 0 else 0.0
        }
