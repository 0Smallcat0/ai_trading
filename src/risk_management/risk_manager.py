...
class RiskManager:
    ...
    def check_stop_loss(self, strategy_name: str, entry_price: float, current_price: float, **kwargs) -> bool:
        """
        檢查停損
        ...
        if should_stop:
            logger.info(f"觸發停損策略 '{strategy_name}': 進場價格 {entry_price}, 當前價格 {current_price}")
    ...
    def check_take_profit(self, strategy_name: str, entry_price: float, current_price: float, **kwargs) -> bool:
        """
        檢查停利
        ...
        # 檢查是否應該停利
        should_take_profit = strategy.should_take_profit(entry_price, current_price, **kwargs)
        
        if should_take_profit:
            logger.info(f"觸發停利策略 '{strategy_name}': 進場價格 {entry_price}, 當前價格 {current_price}")
    ...
    def calculate_position_size(self, strategy_name: str, portfolio_value: float, **kwargs) -> float:
        ...
        logger.info(f"計算倉位大小: 策略 '{strategy_name}', 投資組合價值 {portfolio_value}, 倉位大小 {position_size}")
        
        return position_size
    
    def calculate_shares(self, strategy_name: str, portfolio_value: float, price: float, **kwargs) -> int:
        """
        計算股數
        ...
        logger.info(f"計算股數: 策略 '{strategy_name}', 投資組合價值 {portfolio_value}, 價格 {price}, 股數 {shares}")
        
        return shares
    
    def check_portfolio_limits(self, symbol: str, value: float, sector: Optional[str] = None) -> bool:
        """
        檢查投資組合限制
        ...
        for name, breaker in self.circuit_breakers.items():
            try:
                if breaker.check(**kwargs):
                    ...
                    # 停止交易
                    self.stop_trading(f"熔斷機制 '{name}' 已觸發: {breaker.trigger_reason}")
    ...
    def update_risk_metrics(self, returns: pd.Series, risk_free_rate: float = 0.0) -> Dict[str, float]:
        """
        更新風險指標
    ...
    def _record_risk_event(self, event_type: str, **kwargs) -> None:
        ...
        try:
            ...
            severity = EventSeverity.ERROR if event_type in ["circuit_breaker", "trading_stopped"] else EventSeverity.WARNING
    ...
    def get_risk_events(self, event_type: Optional[str] = None, limit: int = 100) -> List[Dict[str, Any]]:
        """
        獲取風險事件
