...
class KellyPositionSizing(PositionSizingStrategy):
    ...
    def __init__(self, win_rate: float, win_loss_ratio: float, fraction: float = 1.0):
        ...
        logger.info(
            f"初始化凱利公式倉位大小策略: 勝率 {self.win_rate:.2%}, 獲利/虧損比率 {self.win_loss_ratio}, 凱利比例 {self.fraction}"
        )
...
class VolatilityPositionSizing(PositionSizingStrategy):
    ...
    def __init__(self, base_percent: float, volatility_factor: float = 1.0):
        ...
        logger.info(
            f"初始化波動率倉位大小策略: 基礎百分比 {self.base_percent:.2%}, 波動率因子 {self.volatility_factor}"
        )
    ...
    def calculate_position_size(self, portfolio_value: float, **kwargs) -> float:
        ...
        volatility_adjustment = (
            1.0 / (volatility * self.volatility_factor) if volatility > 0 else 1.0
        )
...
class PyramidingPositionSizing(PositionSizingStrategy):
    ...
    def __init__(
        self, initial_percent: float, reduction_factor: float = 0.5, max_levels: int = 3
    ):
        """
        初始化金字塔倉位大小策略
        ...
        self.initial_percent = min(
            abs(initial_percent), 1.0
        )  # 確保初始百分比不超過 100%
        self.reduction_factor = min(
            max(abs(reduction_factor), 0.1), 0.9
        )  # 確保縮減因子在 0.1-0.9 之間
        self.max_levels = max(int(max_levels), 1)  # 確保最大層級數至少為 1
        logger.info(
            f"初始化金字塔倉位大小策略: 初始百分比 {self.initial_percent:.2%}, 縮減因子 {self.reduction_factor}, 最大層級數 {self.max_levels}"
        )
