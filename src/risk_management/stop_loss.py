...
class StopLossStrategy(ABC):
    ...
    @abstractmethod
    def should_stop_out(self, entry_price: float, current_price: float, **kwargs) -> bool:
        """
        判斷是否應該停損
...
class PercentStopLoss(StopLossStrategy):
    ...
    def __init__(self, percent: float, is_long: bool = True):
        ...
        logger.info(f"初始化百分比停損策略: {self.percent:.2%}, {'多頭' if is_long else '空頭'}")
...
class ATRStopLoss(StopLossStrategy):
    ...
    def __init__(self, atr_multiple: float, atr_period: int = 14, is_long: bool = True):
        ...
        self.is_long = is_long
        logger.info(f"初始化 ATR 停損策略: {self.atr_multiple} 倍 ATR, 週期 {self.atr_period}, {'多頭' if is_long else '空頭'}")
...
class TrailingStopLoss(StopLossStrategy):
    ...
    def __init__(self, percent: float, is_long: bool = True):
        ...
        logger.info(f"初始化追蹤停損策略: {self.percent:.2%}, {'多頭' if is_long else '空頭'}")
...
class VolatilityStopLoss(StopLossStrategy):
    ...
    def __init__(self, volatility_multiple: float, lookback_period: int = 20, is_long: bool = True):
        """
        初始化波動率停損策略
        ...
        self.is_long = is_long
        logger.info(f"初始化波動率停損策略: {self.volatility_multiple} 倍波動率, 回顧期間 {self.lookback_period}, {'多頭' if is_long else '空頭'}")
...
class SupportResistanceStopLoss(StopLossStrategy):
    ...
    def __init__(self, is_long: bool = True, buffer_percent: float = 0.01):
        ...
        logger.info(f"初始化支撐阻力停損策略: {'多頭' if is_long else '空頭'}, 緩衝 {self.buffer_percent:.2%}")
