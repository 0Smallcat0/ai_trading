...
class TakeProfitStrategy(ABC):
    ...
    @abstractmethod
    def should_take_profit(self, entry_price: float, current_price: float, **kwargs) -> bool:
        """
        判斷是否應該停利
...
class PercentTakeProfit(TakeProfitStrategy):
    ...
    def __init__(self, percent: float, is_long: bool = True):
        ...
        logger.info(f"初始化百分比停利策略: {self.percent:.2%}, {'多頭' if is_long else '空頭'}")
...
class TargetTakeProfit(TakeProfitStrategy):
    ...
    def __init__(self, target_price: float, is_long: bool = True):
        ...
        self.is_long = is_long
        logger.info(f"初始化目標價停利策略: {self.target_price}, {'多頭' if is_long else '空頭'}")
...
class TrailingTakeProfit(TakeProfitStrategy):
    ...
    def __init__(self, percent: float, is_long: bool = True, activation_percent: float = 0.0):
        """
        初始化追蹤停利策略
        ...
        logger.info(f"初始化追蹤停利策略: {self.percent:.2%}, {'多頭' if is_long else '空頭'}, 啟動百分比: {self.activation_percent:.2%}")
        ...
        if not self.activated:
            if self.is_long:
                ...
                if self.highest_price >= activation_price:
                    self.activated = True
                    logger.info(f"追蹤停利已啟動: 價格 {self.highest_price} >= 啟動價格 {activation_price}")
                else:
                    # 未啟動，返回一個很高的價格
                    return float('inf')
            else:
                ...
                if self.lowest_price <= activation_price:
                    self.activated = True
                    logger.info(f"追蹤停利已啟動: 價格 {self.lowest_price} <= 啟動價格 {activation_price}")
                else:
                    # 未啟動，返回一個很低的價格
                    return 0.0
...
class RiskRewardTakeProfit(TakeProfitStrategy):
    ...
    def __init__(self, risk_reward_ratio: float, is_long: bool = True):
        ...
        self.is_long = is_long
        logger.info(f"初始化風險報酬比停利策略: {self.risk_reward_ratio}, {'多頭' if is_long else '空頭'}")
...
class TimeBasedTakeProfit(TakeProfitStrategy):
    ...
    def __init__(self, target_days: int, min_profit_percent: float = 0.0, is_long: bool = True):
        """
        初始化時間基礎停利策略
        ...
        self.is_long = is_long
        logger.info(f"初始化時間停利策略: 目標持倉 {self.target_days} 天, 最小獲利 {self.min_profit_percent:.2%}, {'多頭' if is_long else '空頭'}")
    ...
    def should_take_profit(self, entry_price: float, current_price: float, **kwargs) -> bool:
        ...
        return holding_days >= self.target_days and profit_percent >= self.min_profit_percent
...
class MultipleTakeProfit(TakeProfitStrategy):
    ...
    def calculate_take_profit(self, entry_price: float, **kwargs) -> float:
        ...
        for strategy in self.strategies:
            try:
                take_profit_price = strategy.calculate_take_profit(entry_price, **kwargs)
