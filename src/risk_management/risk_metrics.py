...
class RiskMetricsCalculator:
    ...
    def __init__(self, returns: pd.Series, risk_free_rate: float = 0.0):
        ...
        logger.info(f"初始化風險指標計算器: 數據點數量 {len(returns)}, 無風險利率 {risk_free_rate:.2%}")
    ...
    def calculate_sortino_ratio(self, annualize: bool = True) -> float:
        ...
        # 計算索提諾比率
        sortino_ratio = mean_excess_return / downside_volatility if downside_volatility > 0 else 0.0
...
class ValueAtRisk:
    ...
    def __init__(self, returns: pd.DataFrame, weights: Optional[Dict[str, float]] = None):
        """
        初始化風險值計算器
        ...
        if weights is None:
            ...
            self.weights = {k: v / total_weight for k, v in weights.items()} if total_weight > 0 else weights
        
        logger.info(f"初始化風險值計算器: 資產數量 {len(returns.columns)}, 數據點數量 {len(returns)}")
    ...
    def calculate_var_monte_carlo(self, confidence: float = 0.95, simulations: int = 10000) -> float:
        """
        使用蒙特卡洛模擬法計算風險值
...
class ConditionalValueAtRisk:
    ...
    def __init__(self, returns: pd.DataFrame, weights: Optional[Dict[str, float]] = None):
        ...
        logger.info(f"初始化條件風險值計算器: 資產數量 {len(returns.columns)}, 數據點數量 {len(returns)}")
    ...
    def calculate_cvar_parametric(self, confidence: float = 0.95) -> float:
        ...
        # 計算條件風險值
        cvar = mean_return - std_return * np.exp(-z**2/2) / ((1 - confidence) * np.sqrt(2 * np.pi))
...
class MaximumDrawdown:
    ...
    def __init__(self, returns: pd.DataFrame, weights: Optional[Dict[str, float]] = None):
        ...
        logger.info(f"初始化最大回撤計算器: 資產數量 {len(returns.columns)}, 數據點數量 {len(returns)}")
...
class VolatilityCalculator:
    ...
    def __init__(self, returns: pd.DataFrame, weights: Optional[Dict[str, float]] = None):
        ...
        logger.info(f"初始化波動率計算器: 資產數量 {len(returns.columns)}, 數據點數量 {len(returns)}")
    ...
    def calculate_rolling_volatility(self, window: int = 20, annualize: bool = True) -> pd.Series:
        """
        計算滾動波動率
