...
class PortfolioRiskManager:
    ...
    def __init__(self, max_position_percent: float = 0.2, max_sector_percent: float = 0.4):
        """
        初始化投資組合風險管理器
        ...
        logger.info(f"初始化投資組合風險管理器: 單一倉位最大 {self.max_position_percent:.2%}, 單一行業最大 {self.max_sector_percent:.2%}")
    
    def add_position(self, symbol: str, value: float, sector: Optional[str] = None) -> bool:
        """
        添加倉位
    ...
    def update_position(self, symbol: str, value: float, sector: Optional[str] = None) -> bool:
        """
        更新倉位
    ...
    def get_position_weights(self) -> Dict[str, float]:
        ...
        return {symbol: position["value"] / portfolio_value for symbol, position in self.positions.items()}
    ...
    def check_position_limit(self, symbol: str, value: float) -> bool:
        ...
        if weight > self.max_position_percent:
            logger.warning(f"倉位 {symbol} 權重 {weight:.2%} 超過限制 {self.max_position_percent:.2%}")
            return False
        
        return True
    
    def check_sector_limit(self, symbol: str, value: float, sector: Optional[str] = None) -> bool:
        """
        檢查行業限制
        ...
        # 計算新的行業總價值
        sector_value = sum(
            position["value"] for s, position in self.positions.items()
        ...
        if weight > self.max_sector_percent:
            logger.warning(f"行業 {sector} 權重 {weight:.2%} 超過限制 {self.max_sector_percent:.2%}")
            return False
        
        return True
    
    def check_all_limits(self, symbol: str, value: float, sector: Optional[str] = None) -> bool:
        """
        檢查所有限制
    ...
    def get_portfolio_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "max_position_weight": max(position_weights.values()) if position_weights else 0.0,
...
class DiversificationManager:
    ...
    def __init__(self, min_stocks: int = 5, max_correlation: float = 0.7):
        ...
        logger.info(f"初始化分散化管理器: 最小股票數量 {self.min_stocks}, 最大相關性 {self.max_correlation}")
    ...
    def check_diversification(self, portfolio: Dict[str, float]) -> bool:
        ...
        if len(portfolio) < self.min_stocks:
            logger.warning(f"投資組合股票數量 {len(portfolio)} 少於最小要求 {self.min_stocks}")
            return False
        
        return True
    
    def suggest_diversification(self, portfolio: Dict[str, float], candidates: List[str]) -> List[str]:
        """
        建議分散化
...
class CorrelationAnalyzer:
    ...
    def __init__(self, price_data: Dict[str, pd.DataFrame], lookback_period: int = 252):
        ...
        logger.info(f"初始化相關性分析器: 股票數量 {len(price_data)}, 回顧期間 {lookback_period}")
    ...
    def get_correlation(self, symbol1: str, symbol2: str) -> float:
        ...
        Returns:
            float: 相關性係數
        """
        if symbol1 not in self.correlation_matrix.index or symbol2 not in self.correlation_matrix.columns:
            return 0.0
...
class RiskParityStrategy:
    ...
    def __init__(self, price_data: Dict[str, pd.DataFrame], lookback_period: int = 252):
        ...
        logger.info(f"初始化風險平價策略: 股票數量 {len(price_data)}, 回顧期間 {lookback_period}")
    ...
    def calculate_weights(self) -> Dict[str, float]:
        ...
        # 轉換為字典
        weight_dict = {symbol: weight for symbol, weight in zip(returns.keys(), weights)}
...
class SectorExposureManager:
    ...
    def __init__(self, sector_limits: Dict[str, float]):
        ...
        """
        self.sector_limits = {sector: min(abs(limit), 1.0) for sector, limit in sector_limits.items()}
        
        logger.info(f"初始化行業暴露管理器: 行業限制 {self.sector_limits}")
    
    def check_sector_exposure(self, portfolio: Dict[str, Dict[str, Any]]) -> Dict[str, bool]:
        """
        檢查行業暴露
        ...
        # 計算行業權重
        sector_weights = {sector: value / portfolio_value for sector, value in sector_values.items()} if portfolio_value > 0 else {}
        
        # 檢查是否符合限制
        results = {}
        for sector, weight in sector_weights.items():
            limit = self.sector_limits.get(sector, 1.0)  # 如果沒有設置限制，則默認為 100%
            results[sector] = weight <= limit
        
        return results
    
    def get_sector_adjustments(self, portfolio: Dict[str, Dict[str, Any]]) -> Dict[str, float]:
        """
        獲取行業調整建議
...
class ConcentrationRiskManager:
    ...
    def __init__(self, max_position_percent: float = 0.2, max_positions: int = 20):
        ...
        logger.info(f"初始化集中度風險管理器: 單一倉位最大 {self.max_position_percent:.2%}, 最大倉位數量 {self.max_positions}")
    ...
    def check_concentration(self, portfolio: Dict[str, float]) -> bool:
        ...
        if len(portfolio) > self.max_positions:
            logger.warning(f"投資組合倉位數量 {len(portfolio)} 超過最大限制 {self.max_positions}")
            return False
    ...
    def get_concentration_adjustments(self, portfolio: Dict[str, float]) -> Dict[str, float]:
        """
        獲取集中度調整建議
