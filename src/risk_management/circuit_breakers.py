...
class DrawdownCircuitBreaker(CircuitBreaker):
    ...
    def __init__(self, max_drawdown: float, lookback_period: int = 0):
        ...
        logger.info(f"初始化回撤熔斷機制: 最大回撤 {self.max_drawdown:.2%}, 回顧期間 {self.lookback_period}")
    ...
    def check(self, **kwargs) -> bool:
        ...
        if abs(max_drawdown) > self.max_drawdown:
            self.trigger(f"回撤 {abs(max_drawdown):.2%} 超過閾值 {self.max_drawdown:.2%}")
            return True
...
class VolatilityCircuitBreaker(CircuitBreaker):
    ...
    def __init__(self, max_volatility: float, window: int = 20, annualize: bool = True):
        ...
        logger.info(f"初始化波動率熔斷機制: 最大波動率 {self.max_volatility:.2%}, 窗口大小 {self.window}, 年化 {self.annualize}")
...
class LossCircuitBreaker(CircuitBreaker):
    ...
    def __init__(self, max_daily_loss: float, max_weekly_loss: float = 0.0, max_monthly_loss: float = 0.0):
        """
        初始化虧損熔斷機制
        ...
        logger.info(f"初始化虧損熔斷機制: 最大日虧損 {self.max_daily_loss:.2%}, 最大週虧損 {self.max_weekly_loss:.2%}, 最大月虧損 {self.max_monthly_loss:.2%}")
        ...
        if returns is not None:
            ...
            if len(returns) > 0 and self.max_daily_loss > 0:
                ...
                if daily_return < 0 and abs(daily_return) > self.max_daily_loss:
                    self.trigger(f"日虧損 {abs(daily_return):.2%} 超過閾值 {self.max_daily_loss:.2%}")
                    return True
            ...
            if len(returns) >= 5 and self.max_weekly_loss > 0:
                ...
                if weekly_return < 0 and abs(weekly_return) > self.max_weekly_loss:
                    self.trigger(f"週虧損 {abs(weekly_return):.2%} 超過閾值 {self.max_weekly_loss:.2%}")
                    return True
            ...
            if len(returns) >= 21 and self.max_monthly_loss > 0:
                ...
                if monthly_return < 0 and abs(monthly_return) > self.max_monthly_loss:
                    self.trigger(f"月虧損 {abs(monthly_return):.2%} 超過閾值 {self.max_monthly_loss:.2%}")
                    return True
...
class TimeCircuitBreaker(CircuitBreaker):
    ...
    def __init__(self, start_time: str, end_time: str, days: Optional[List[int]] = None):
        """
        初始化時間熔斷機制
        ...
        logger.info(f"初始化時間熔斷機制: 開始時間 {self.start_time}, 結束時間 {self.end_time}, 生效日 {self.days}")
    ...
    def check(self, **kwargs) -> bool:
        ...
        end_datetime = current_time.replace(hour=end_hour, minute=end_minute, second=0, microsecond=0)
        
        # 檢查是否在時間範圍內
        if start_datetime <= current_time <= end_datetime:
            self.trigger(f"當前時間 {current_time.strftime('%H:%M')} 在熔斷時間範圍內 ({self.start_time} - {self.end_time})")
            return True
...
class CompositeCircuitBreaker(CircuitBreaker):
        ...
        for breaker in self.circuit_breakers:
            try:
                if breaker.check(**kwargs):
                    self.trigger(f"熔斷機制 '{breaker.name}' 已觸發: {breaker.trigger_reason}")
                    return True
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        # 添加所有熔斷機制的狀態
        status["circuit_breakers"] = [breaker.get_status() for breaker in self.circuit_breakers]
