...
from src.core.features import calculate_technical_indicators, calculate_fundamental_indicators
...
class TimeSeriesSplit:
    ...
    def split(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        logger.info(f"資料集分割完成: 訓練集 {len(train)} 筆, 驗證集 {len(val)} 筆, 測試集 {len(test)} 筆")
    ...
    def split_by_date(
        ...
        val = df[(df[self.date_column] > train_end_date) & (df[self.date_column] <= val_end_date)].copy()
        test = df[df[self.date_column] > val_end_date].copy()
        
        logger.info(f"資料集分割完成: 訓練集 {len(train)} 筆 (截至 {train_end_date}), "
...
class FeatureProcessor:
    ...
    def __init__(
        ...
        if scaler_type == "standard":
            ...
            raise ValueError(f"未知的縮放器類型: {scaler_type}，可用類型: standard, minmax")
    ...
    def transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if missing_columns:
            ...
            # 轉換特徵
            df_scaled = df.copy()
            df_scaled[available_columns] = self.scaler.transform(temp_df[self.feature_columns])[
...
class DatasetLoader:
    ...
    def prepare_features(
        ...
        if target_type == "return":
            # 計算未來 n 天的收益率
            features["target"] = features.groupby(self.symbol_column)[self.price_column].shift(-target_horizon) / \
                                features[self.price_column] - 1
        elif target_type == "direction":
            # 計算未來 n 天的價格方向（上漲=1，下跌=0）
            future_price = features.groupby(self.symbol_column)[self.price_column].shift(-target_horizon)
            features["target"] = (future_price > features[self.price_column]).astype(int)
        elif target_type == "volatility":
            # 計算未來 n 天的波動率
            future_prices = [features.groupby(self.symbol_column)[self.price_column].shift(-i) 
                            for i in range(1, target_horizon + 1)]
            future_returns = [(p / features[self.price_column] - 1) for p in future_prices]
            features["target"] = pd.concat(future_returns, axis=1).std(axis=1)
        else:
            logger.error(f"未知的目標類型: {target_type}")
            raise ValueError(f"未知的目標類型: {target_type}，可用類型: return, direction, volatility")
        
        # 處理缺失值
        features = features.dropna()
        
        logger.info(f"特徵資料集準備完成: {len(features)} 筆, {len(features.columns)} 個特徵")
