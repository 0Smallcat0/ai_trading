...
def validate_explanation_inputs(
    X: Any, y: Optional[Any] = None, feature_names: Optional[List[str]] = None
) -> None:
    """
    驗證解釋輸入資料格式
...
def plot_explanation_comparison(
    ...
    try:
        ...
        for idx, (method, results) in enumerate(results_dict.items()):
            ...
            # 繪製特徵重要性（如果有）
            if "importance" in results and isinstance(
                ...
                ax.set_title(
                    f"{method.upper()} Feature Importance",
                    fontsize=12,
                    fontweight="bold",
                )
...
def create_interpreter(
    model: ModelBase, interpreter_type: str = "default", **kwargs: Any
) -> Any:
    """
    創建解釋器的工廠函數
