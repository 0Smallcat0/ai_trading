...
class FeatureImportanceAnalyzer:
            ...
            for result_type, result_data in results.items():
                if isinstance(result_data, dict) and "importance" in result_data:
                    saved_paths = save_explanation_results(
                        result_data,
                        self.output_dir,
                        f"feature_importance_{result_type}",
                        log_to_mlflow,
                    )
    ...
    def _calculate_permutation_importance(
        self, X: Any, y: Any, n_repeats: int, scoring: Optional[str], **kwargs: Any
    ) -> Dict[str, Any]:
        """
        計算排列重要性
        ...
        feature_names = self.feature_names or [
            f"feature_{i}" for i in range(len(result.importances_mean))
        ]
    ...
    def _create_importance_plot(
        self, importance_df: pd.DataFrame, title: str, method: str
    ) -> str:
        """
        創建重要性視覺化圖表
        ...
        try:
            ...
            plot_path = os.path.join(
                self.output_dir, f"feature_importance_{method}.png"
            )
    ...
    def _create_comparison_plot(
        self, perm_results: Dict[str, Any], model_results: Dict[str, Any]
    ) -> str:
        """
        創建重要性方法比較圖
        ...
        try:
            ...
            # 保存比較圖
            plot_path = os.path.join(
                self.output_dir, "feature_importance_comparison.png"
            )
            plt.savefig(plot_path, dpi=300, bbox_inches="tight")
            plt.close()
