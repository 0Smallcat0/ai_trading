        ...
        try:
            ...
            if log_to_mlflow:
                ...
                if not self.feature_importance.empty:
                    ...
                    sns.barplot(
                        x="importance", y="feature", data=self.feature_importance, ax=ax
                    )
                ...
                if hasattr(self.model.model, "save_model"):
                    ...
                    if hasattr(mlflow, self.model.__class__.__module__.split(".")[-1]):
                        getattr(
                            mlflow, self.model.__class__.__module__.split(".")[-1]
                        ).log_model(self.model.model, "model")
                    else:
            ...
            # 檢查模型是否達到接受標準
            accepted = self._check_acceptance_criteria(
                self.val_metrics or self.train_metrics
            )
            if accepted:
        ...
        **train_params,
        ...
        try:
            ...
            for metric in cv_results[0]["train_metrics"].keys():
                avg_train_metrics[metric] = np.mean(
                    [r["train_metrics"][metric] for r in cv_results]
                )
                avg_val_metrics[metric] = np.mean(
                    [r["val_metrics"][metric] for r in cv_results]
                )
        ...
        for metric, threshold in self.metrics_threshold.items():
            ...
            if metric in ["max_drawdown"]:
                ...
                if metrics[metric] < threshold:
                    logger.warning(
                        f"指標 {metric} = {metrics[metric]} 未達到閾值 {threshold}"
                    )
                    return False
            else:
