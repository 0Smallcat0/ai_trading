...
class ModelInterpreter:
    ...
    def explain_with_shap(
        ...
        **kwargs,
            ...
            try:
                ...
                self.shap_explainer = shap.KernelExplainer(
                    (
                        ...
                        if hasattr(self.model.model, "predict")
        ...
        if plot_type == "summary":
            shap.summary_plot(
                self.shap_values, X_sample, feature_names=self.feature_names, show=False
            )
        elif plot_type == "bar":
            shap.summary_plot(
                self.shap_values,
        ...
        importance_df = pd.DataFrame(
            {
                ...
                or [f"feature_{i}" for i in range(len(shap_importance))],
    ...
    def explain_with_lime(
        ...
        **kwargs,
        ...
        if y is not None:
            ...
            if num_correct > 0:
                correct_samples = np.random.choice(
                    correct_idx, num_correct, replace=False
                )
            else:
                correct_samples = []
            ...
            if mode == "classification":
                explanation = self.lime_explainer.explain_instance(
                    ...
                    (
                        ...
                        else lambda x: self.model.model.predict_proba(x)
                    ),
                ...
                explanation = self.lime_explainer.explain_instance(
                    ...
                    (
                        ...
                        else lambda x: self.model.model.predict(x)
                    ),
                    num_features=num_features,
                    num_samples=num_samples,
                )
        ...
        return {"lime_explainer": self.lime_explainer, "explanations": explanations}
        ...
        **kwargs,
        ...
        if method == "permutation":
            ...
            result = permutation_importance(
                self.model.model, X, y, n_repeats=n_repeats, random_state=42, **kwargs
            )
            ...
            importance_df = pd.DataFrame(
                {
                    "feature": self.feature_names
                    or [f"feature_{i}" for i in range(len(result.importances_mean))],
        ...
        importance_path = os.path.join(
            self.output_dir, f"feature_importance_{method}.csv"
        )
        importance_df.to_csv(importance_path, index=False)
