...
def demonstrate_missing_value_handling():
    ...
    for i, method in enumerate(methods):
        ...
        plt.plot(df.index, df['close'], 'o-', label='Original (with missing)', alpha=0.5)
        plt.plot(results[method].index, results[method]['close'], 'o-', label=f'Filled ({method})')
...
def demonstrate_outlier_handling():
    ...
    for treatment_method in treatment_methods:
        handler = OutlierHandler(detection_method=detection_method, treatment_method=treatment_method)
    ...
    for i, method in enumerate(treatment_methods):
        ...
        plt.plot(df.index, df['close'], 'o-', label='Original (with outliers)', alpha=0.5)
        plt.plot(results[method].index, results[method]['close'], 'o-', label=f'Treated ({method})')
...
def demonstrate_price_standardization():
    ...
    # 創建包含調整後收盤價的示例資料
    df = create_sample_data(with_missing=False, with_outliers=False, with_adj_close=True)
    ...
    plt.subplot(1, 2, 2)
    plt.plot(standardized_df.index, standardized_df['close'], 'o-', label='Standardized Close')
