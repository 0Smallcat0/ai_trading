...
def create_sample_data():
    ...
    for stock in stocks:
        ...
        # 生成隨機漫步
        steps = np.random.normal(0, 1, len(dates)) * start_price * 0.01  # 每日波動約為 1%
        # 累積步驟得到價格序列
...
def demonstrate_feature_selection(calculator, features_df):
    ...
    selected_df_rfe, _ = calculator.select_features(
        features_df, method='rfe', k=10
    )
    logger.info(f"RFE 選擇的特徵: {selected_df_rfe.columns.tolist()}")
    
    # 使用 Lasso 選擇特徵
    logger.info("使用 Lasso 選擇特徵")
    selected_df_lasso, _ = calculator.select_features(
        features_df, method='lasso', k=10
    )
    logger.info(f"Lasso 選擇的特徵: {selected_df_lasso.columns.tolist()}")
    
    # 比較不同方法選擇的特徵
    all_selected = set(selected_df_f.columns) | set(selected_df_rfe.columns) | set(selected_df_lasso.columns)
    common_selected = set(selected_df_f.columns) & set(selected_df_rfe.columns) & set(selected_df_lasso.columns)
...
def main():
    ...
    # 展示特徵選擇
    selected_features = demonstrate_feature_selection(calculator, features)
    
    # 展示降維
    reduced_features = demonstrate_dimensionality_reduction(calculator, features)
    
    # 展示特徵重要性
    importance = demonstrate_feature_importance(calculator, features)
    
    # 展示特徵存儲
    version = demonstrate_feature_store(features)
