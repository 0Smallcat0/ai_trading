...
from src.streaming import (
    StreamManager, Message, MessageType, MessagePriority,
    Producer, WebSocketProducer, KafkaProducer,
    Consumer, WebSocketConsumer, KafkaConsumer,
    Processor, FeatureProcessor, ModelProcessor,
    Pipeline, StreamingPipeline, FeaturePipeline, ModelPipeline,
    StreamMonitor
)
...
def calculate_features(data: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    try:
        ...
        if all(col in df.columns for col in ["price", "volume"]):
            features["vwap"] = calculate_vwap(df["price"].values, df["volume"].values)[-1]
...
class TradingSignalProcessor(Processor):
    ...
    def process_message(self, message: Message) -> Optional[Message]:
        ...
        if confidence < self.confidence_threshold:
            logger.info(f"股票 {symbol} 的置信度 {confidence:.2f} 低於閾值 {self.confidence_threshold:.2f}，不生成交易信號")
            return None
