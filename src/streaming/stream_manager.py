...
class StreamManager:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        # 消息隊列
        self.message_queue = queue.PriorityQueue(maxsize=self.config.get("queue_size", 10000))
    ...
    def subscribe(self, processor_name: str, message_types: List[MessageType]) -> bool:
        ...
        logger.info(f"處理器 '{processor_name}' 已訂閱消息類型: {[mt.name for mt in message_types]}")
        return True
    
    def unsubscribe(self, processor_name: str, message_types: Optional[List[MessageType]] = None) -> bool:
        """
        取消訂閱消息類型
        ...
        # 取消特定訂閱
        for message_type in message_types:
            if message_type in self.subscriptions and processor_name in self.subscriptions[message_type]:
                self.subscriptions[message_type].remove(processor_name)
        
        logger.info(f"處理器 '{processor_name}' 已取消訂閱消息類型: {[mt.name for mt in message_types]}")
        return True
        ...
        if message.destination and message.destination in self.processors:
            if message.destination in processor_names:
                ...
                try:
                    ...
                    logger.error(f"處理器 '{message.destination}' 處理消息時發生錯誤: {e}")
            return
