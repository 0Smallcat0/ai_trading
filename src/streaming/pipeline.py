...
class Pipeline(ABC):
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # 獲取處理器統計信息
        processor_stats = {
            processor.name: processor.get_stats()
            for processor in self.processors
        }
...
class StreamingPipeline(Pipeline):
    ...
    def add_processor(self, processor: Processor, input_types: Optional[List[MessageType]] = None) -> bool:
        """
        添加處理器
...
class FeaturePipeline(StreamingPipeline):
    ...
    def __init__(
        ...
        logger.info(f"特徵處理管道 '{name}' 已初始化，處理器數量: {len(feature_processors)}")
...
class ModelPipeline(StreamingPipeline):
    ...
    def __init__(
        ...
        logger.info(f"模型處理管道 '{name}' 已初始化，處理器數量: {len(model_processors)}")
...
class TradingPipeline(StreamingPipeline):
    ...
    def __init__(
        ...
        logger.info(f"交易處理管道 '{name}' 已初始化，處理器數量: {len(trading_processors)}")
