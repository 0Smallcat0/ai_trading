...
class BackfillManager:
    ...
    def __init__(self, config: BackfillConfig, data_manager=None):
        ...
        # 設定快取目錄
        self.cache_dir = (
            Path(config.cache_dir) if config.cache_dir 
            else Path("data/cache/backfill")
        )
    ...
    def comprehensive_backfill_with_validation(
        ...
        try:
            ...
            if enable_validation:
                result = self._perform_continuity_check(
            ...
            logger.info(
                "綜合歷史資料回填完成，耗時 %.2f 秒", result.execution_time
            )
    ...
    def _filter_symbols_for_update(
        ...
        """過濾出需要更新的股票。"""
        symbols_to_update = [
            symbol for symbol, info in result.incremental_info.items() 
    ...
    def _perform_outlier_detection(
        ...
        outlier_results = self.detector.detect_outliers_comprehensive(
            result.data
        )
