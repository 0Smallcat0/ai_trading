...
class ModelSignalOptimizer:
            ...
            if method == "moving_average":
                ...
                if len(stock_signals) >= window:
                    try:
                        ...
                        logger.warning(
                            f"Savitzky-Golay 濾波失敗: {e}，使用移動平均替代"
                        )
                        smoothed = stock_signals.rolling(
    ...
    def resolve_conflicts(
        ...
        if method == "majority_vote":
            ...
            for idx in common_index:
                votes = [
                    signals.loc[idx, "signal"] for signals in signals_dict.values()
                ]
            ...
            if weights is None:
                ...
                priority_list = sorted(
                    weights.keys(), key=lambda x: weights.get(x, 0), reverse=True
                )
    ...
    def optimize_combination(
        ...
        for name, signals in signals_dict.items():
            # 計算訊號的收益率
            returns = self._calculate_signal_returns(
                signals.loc[common_index], price_data.loc[common_index]
            )
            ...
            if optimization_metric == "sharpe_ratio":
                ...
                logger.warning(
                    f"未知的優化指標: {optimization_metric}，使用夏普比率替代"
                )
                performance[name] = self._calculate_sharpe_ratio(returns)
        ...
        weights = {
            name: max(0.1, perf) / total_performance
            for name, perf in performance.items()
        }

        # 使用最佳權重組合訊號
        optimized_signals = self.resolve_conflicts(
            signals_dict, method="weighted_average", weights=weights
        )
