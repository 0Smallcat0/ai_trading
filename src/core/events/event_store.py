...
class EventStore:
    ...
    def _init_db(self):
        ...
        try:
            ...
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_events_subject ON events (subject)')
    ...
    def _store_event(self, event: Event):
        ...
        try:
            ...
            # 將字典和列表轉換為JSON字符串
            data = json.dumps(event_dict["data"], ensure_ascii=False) if event_dict["data"] else None
            tags = json.dumps(event_dict["tags"], ensure_ascii=False) if event_dict["tags"] else None
            related_events = json.dumps(event_dict["related_events"], ensure_ascii=False) if event_dict["related_events"] else None
    ...
    def get_event(self, event_id: str) -> Optional[Event]:
        ...
        try:
            ...
            # 將JSON字符串轉換為字典和列表
            event_data["data"] = json.loads(event_data["data"]) if event_data["data"] else {}
            event_data["tags"] = json.loads(event_data["tags"]) if event_data["tags"] else []
            event_data["related_events"] = json.loads(event_data["related_events"]) if event_data["related_events"] else []
