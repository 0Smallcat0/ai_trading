...
class TypeFilter(EventFilter):
    ...
    def __init__(self, name: str, include_types: List[EventType], exclude_types: Optional[List[EventType]] = None):
        """
        初始化類型過濾器
        ...
        logger.info(f"類型過濾器 '{name}' 已初始化，包含類型: {[t.name for t in include_types]}, 排除類型: {[t.name for t in exclude_types] if exclude_types else '無'}")
...
class SeverityFilter(EventFilter):
    ...
    def __init__(self, name: str, min_severity: EventSeverity, event_types: Optional[List[EventType]] = None):
        """
        初始化嚴重程度過濾器
        ...
        logger.info(f"嚴重程度過濾器 '{name}' 已初始化，最小嚴重程度: {min_severity.name}")
...
class SourceFilter(EventFilter):
    ...
    def __init__(self, name: str, include_sources: List[EventSource], exclude_sources: Optional[List[EventSource]] = None, event_types: Optional[List[EventType]] = None):
        """
        初始化來源過濾器
        ...
        logger.info(f"來源過濾器 '{name}' 已初始化，包含來源: {[s.name for s in include_sources]}, 排除來源: {[s.name for s in exclude_sources] if exclude_sources else '無'}")
...
class SubjectFilter(EventFilter):
    ...
    def __init__(self, name: str, patterns: List[Union[str, Pattern]], event_types: Optional[List[EventType]] = None):
        """
        初始化主題過濾器
...
class CompositeFilter(EventFilter):
    ...
    def __init__(self, name: str, filters: List[EventFilter], require_all: bool = False):
        """
        初始化複合過濾器
        ...
        logger.info(f"複合過濾器 '{name}' 已初始化，過濾器數量: {len(filters)}, 要求全部通過: {require_all}")
...
class DataFilter(EventFilter):
    ...
    def __init__(self, name: str, conditions: Dict[str, Any], event_types: Optional[List[EventType]] = None):
        """
        初始化數據過濾器
    ...
    def filter(self, event: Event) -> bool:
        ...
        for key, expected_value in self.conditions.items():
            ...
            if callable(expected_value):
                ...
                # 如果是字典，則檢查是否為子集
                if not all(k in actual_value and actual_value[k] == v for k, v in expected_value.items()):
                    return False
            else:
                # 否則直接比較值
