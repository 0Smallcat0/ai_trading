...
class SequenceCorrelator(EventCorrelator):
    ...
    def __init__(self, name: str, sequence: List[EventType], window_size: int = 300, generate_event: bool = True):
        """
        初始化序列關聯分析器
        ...
        logger.info(f"序列關聯分析器 '{name}' 已初始化，序列: {[s.name for s in sequence]}")
    ...
    def correlate(self, event: Event) -> Optional[Event]:
        ...
        if expected_index < len(self.sequence) and event.event_type == self.sequence[expected_index]:
            # 添加到序列事件
    ...
    def _create_correlated_event(self) -> Event:
        ...
        return Event(
            ...
            data={
                ...
                "subjects": [event.subject for event in self.sequence_events if event.subject]
            },
...
class SubjectCorrelator(EventCorrelator):
    ...
    def __init__(self, name: str, event_types: List[EventType], window_size: int = 300, threshold: int = 3):
        """
        初始化主題關聯分析器
    ...
    def correlate(self, event: Event) -> Optional[Event]:
        ...
        # 檢查是否需要關聯
        if (
            len(self.subject_events[event.subject]) >= self.threshold and
            (event.subject not in self.last_correlation_time or
             current_time - self.last_correlation_time[event.subject] >= self.window_size)
        ):
            # 獲取相同主題的事件
            subject_events = self.subject_events[event.subject]
            
            # 生成關聯事件
            correlated_event = self._create_correlated_event(event.subject, subject_events)
...
class RuleBasedCorrelator(EventCorrelator):
    ...
    def __init__(self, name: str, event_types: List[EventType], rules: List[Callable[[Event, List[Event]], bool]], window_size: int = 300):
        """
        初始化規則基礎關聯分析器
    ...
    def correlate(self, event: Event) -> Optional[List[Event]]:
        ...
        for rule in self.rules:
            ...
            if rule(event, buffer_events):
                # 創建關聯事件
                correlated_event = self._create_rule_event(event, buffer_events, self.rules.index(rule))
                correlated_events.append(correlated_event)
        
        return correlated_events if correlated_events else None
    
    def _create_rule_event(self, event: Event, buffer_events: List[Event], rule_index: int) -> Event:
        """
        創建規則關聯事件
