...
class CountAggregator(EventAggregator):
    ...
    def __init__(self, name: str, event_types: List[EventType], window_size: int = 60, threshold: int = 10):
        """
        初始化計數聚合器
    ...
    def aggregate(self) -> Optional[Event]:
        ...
        if count >= self.threshold:
            ...
            return Event(
                ...
                related_events=[event.id for event in self.events[:10]]  # 只包含前10個相關事件
            )
...
class SubjectAggregator(EventAggregator):
    ...
    def __init__(self, name: str, event_types: List[EventType], window_size: int = 60, threshold: int = 5):
        """
        初始化主題聚合器
...
class SeverityAggregator(EventAggregator):
    ...
    def __init__(self, name: str, event_types: List[EventType], window_size: int = 60, min_severity: EventSeverity = EventSeverity.ERROR, threshold: int = 3):
        """
        初始化嚴重程度聚合器
        ...
        logger.info(f"嚴重程度聚合器 '{name}' 已初始化，最小嚴重程度: {min_severity.name}，閾值: {threshold}")
    ...
    def aggregate(self) -> Optional[Event]:
        ...
        severe_events = [
            ...
            if self.severity_order[event.severity] >= self.severity_order[self.min_severity]
        ]
        ...
        if len(severe_events) >= self.threshold:
            ...
            return Event(
                ...
                related_events=[event.id for event in severe_events[:10]]  # 只包含前10個相關事件
            )
