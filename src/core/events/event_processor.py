...
class EventProcessor(ABC):
    ...
    def get_stats(self):
        ...
        return {
            ...
            "event_types": (
                [et.name for et in self.event_types] if self.event_types else "all"
            ),
        }
...
class CompositeEventProcessor(EventProcessor):
    ...
    def __init__(self, name: str, processors: List[EventProcessor]):
        ...
        logger.info(
            f"複合事件處理器 '{name}' 已初始化，包含 {len(processors)} 個處理器"
        )
...
class EventProcessorRegistry:
    ...
    def get_stats(self):
        ...
        """
        return {
            name: processor.get_stats() for name, processor in self.processors.items()
        }
