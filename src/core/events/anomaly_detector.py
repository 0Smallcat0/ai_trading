...
class FrequencyAnomalyDetector(AnomalyDetector):
    ...
    def _check_anomaly(
        self, event_key: Tuple[EventType, EventSource], event: Event
    ) -> Optional[Event]:
        """
        檢查事件頻率是否異常
...
class PatternAnomalyDetector(AnomalyDetector):
    ...
    def detect_anomaly(self, event: Event) -> Optional[Event]:
        ...
        current_pattern = tuple(
            e.event_type for e in list(self.event_buffer)[-self.pattern_size :]
        )
    ...
    def _check_anomaly(
        self, pattern: Tuple[EventType, ...], event: Event
    ) -> Optional[Event]:
        """
        檢查事件模式是否異常
...
class ValueAnomalyDetector(AnomalyDetector):
    ...
    def __init__(
        ...
        logger.info(
            f"數值異常檢測器 '{name}' 已初始化，數據字段: {data_field}，閾值: {threshold}"
        )
    ...
    def _check_anomaly(
        self, subject: str, value: float, event: Event
    ) -> Optional[Event]:
        """
        檢查數值是否異常
