...
class HybridSignalGenerator(SignalGenerator):
    ...
    def generate_hybrid_signals(
        ...
        """
        # 生成 AI 模型訊號
        ai_signals = self.generate_ai_model_signals(
            model_name, version, signal_threshold
        )
        ...
        # 合併規則型策略訊號
        combined_rule_signals = self.combine_signals(
            {strategy: 1.0 / len(rule_strategies) for strategy in rule_strategies}
        )

        # 生成混合訊號
        hybrid_signals = self._combine_ai_rule_signals(
            ai_signals, combined_rule_signals
        )
    ...
    def _combine_ai_rule_signals(
        self, ai_signals: pd.DataFrame, rule_signals: pd.DataFrame
    ) -> pd.DataFrame:
        """
        合併 AI 模型訊號與規則型策略訊號
        ...
        if self.conflict_resolution == "ai_priority":
            ...
            hybrid_signals.loc[ai_signals["signal"] == 0, "signal"] = rule_signals.loc[
                ai_signals["signal"] == 0, "signal"
            ]
        elif self.conflict_resolution == "rule_priority":
            # 規則型優先
            hybrid_signals["signal"] = rule_signals["signal"]
            # 只在規則型訊號為 0 時使用 AI 訊號
            hybrid_signals.loc[rule_signals["signal"] == 0, "signal"] = ai_signals.loc[
                rule_signals["signal"] == 0, "signal"
            ]
        else:
            # 加權合併
            # 標準化權重
            ...
            hybrid_signals["signal"] = (
                ai_signals["signal"] * ai_norm_weight
                + rule_signals["signal"] * rule_norm_weight
            )
        ...
        for stock_id in signals.index.get_level_values(0).unique():
            ...
            for i in range(lookback_window, len(stock_signals)):
                ...
                if buy_count >= confirmation_threshold:
                    optimized_signals.loc[
                        (stock_id, stock_signals.index[i]), "signal"
                    ] = 1
    ...
    def adjust_signal_strength(
        ...
        for stock_id in signals.index.get_level_values(0).unique():
            ...
            # 計算波動率因子
            volatility = stock_prices.pct_change().rolling(window=20).std() * np.sqrt(
                252
            )  # 年化波動率
            volatility_factor = (volatility / volatility.mean() - 1).clip(
                -1, 1
            )  # 限制在 [-1, 1] 範圍內
            ...
            for i in range(len(stock_signals)):
                if stock_signals.iloc[i] != 0:
                    ...
                    if signal_idx in strength.index:
                        ...
                        if (direction > 0 and strength_value > 0) or (
                            direction < 0 and strength_value < 0
                        ):
                            # 訊號增強，但仍保持原方向
                            adjusted_signals.loc[(stock_id, signal_idx), "signal"] = (
                                direction
                            )
