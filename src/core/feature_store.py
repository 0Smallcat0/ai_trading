...
class FeatureStore:
    ...
    def __init__(self, base_dir="data/features", use_mlflow=False, mlflow_tracking_uri=None):
        """
        初始化特徵存儲
    ...
    def save_features(self, features_df: pd.DataFrame, name: str, metadata: Dict = None, 
                     version: str = None, tags: List[str] = None) -> str:
        """
        保存特徵
    ...
    def load_features(self, name: str, version: str = "latest") -> Tuple[pd.DataFrame, Dict]:
        """
        載入特徵
    ...
    def list_versions(self, name: str) -> List[str]:
        ...
        return [d for d in os.listdir(feature_dir) 
    ...
    def search_features(self, tags: List[str] = None, name_pattern: str = None) -> List[Dict]:
        """
        搜索特徵
    ...
    def _log_to_mlflow(self, features_df: pd.DataFrame, name: str, version: str, 
                      metadata: Dict, tags: List[str] = None) -> None:
        """
        將特徵記錄到 MLflow
        ...
        try:
            with mlflow.start_run(experiment_id=experiment_id, run_name=version):
                ...
                try:
                    ...
                    mlflow.log_params({f"stats_{k}": str(v) for k, v in stats_dict.items()})
                except Exception as e:
