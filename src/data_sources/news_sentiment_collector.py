...
class NewsSentimentCollector(DataCollector):
    ...
    def _init_sentiment_model(self):
        ...
        if self.sentiment_model == "simple":
            ...
            logger.warning(f"不支援的情緒分析模型: {self.sentiment_model}，使用簡單模型")
            self.sentiment_model = "simple"
            self._init_sentiment_model()
    ...
    def analyze_sentiment(self, text: str) -> Dict[str, Any]:
        ...
        if self.sentiment_model == "simple":
            ...
            return {
                ...
                "neutral_count": 1 if total == 0 else 0,
            }
            ...
            logger.error(f"不支援的情緒分析模型: {self.sentiment_model}")
            return {
                "sentiment_score": 0.0,
                "sentiment_label": "neutral",
                "positive_count": 0,
    ...
    def collect_news(
        ...
        """
        logger.info(f"開始收集 {len(symbols)} 檔股票的新聞資料")
            ...
            for future in as_completed(future_to_symbol):
                ...
                try:
                    ...
                    if news_list:
                        ...
                        logger.info(f"成功收集 {symbol} 的新聞資料，共 {len(news_list)} 筆")
                        
                        # 儲存到資料庫
                        if save_to_db:
                            self._save_news_to_db(symbol, news_list)
                    else:
                        logger.warning(f"收集 {symbol} 的新聞資料為空")
                except Exception as e:
                    logger.error(f"收集 {symbol} 的新聞資料時發生錯誤: {e}")

        logger.info(f"完成收集新聞資料，成功收集 {len(results)} 檔股票")
        return results
    ...
    def _collect_news_for_symbol(self, symbol: str, days: int) -> List[Dict[str, Any]]:
        ...
        if self.use_cache and self._is_cache_valid(cache_path):
            logger.info(f"從快取讀取 {symbol} 的新聞資料")
        ...
        if self.source == "mcp":
            try:
                ...
                logger.error(f"從 MCP 獲取 {symbol} 的新聞資料時發生錯誤: {e}")
                return []
        else:
    ...
    def _save_news_to_db(self, symbol: str, news_list: List[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            news_by_date = {}
            for news in news_list:
                news_date = datetime.strptime(news["date"], "%Y-%m-%d").date() if isinstance(news["date"], str) else news["date"]
            ...
            for news_date, daily_news in news_by_date.items():
                # 計算情緒統計
                sentiment_scores = [news.get("sentiment_score", 0) for news in daily_news if "sentiment_score" in news]
                positive_count = sum(news.get("positive_count", 0) for news in daily_news if "positive_count" in news)
                negative_count = sum(news.get("negative_count", 0) for news in daily_news if "negative_count" in news)
                neutral_count = sum(news.get("neutral_count", 0) for news in daily_news if "neutral_count" in news)
                
                # 計算平均情緒分數
                avg_sentiment = sum(sentiment_scores) / len(sentiment_scores) if sentiment_scores else 0
                
                # 生成新聞摘要
                news_summary = "\n".join([f"{i+1}. {news.get('title', 'No Title')}" for i, news in enumerate(daily_news[:5])])
                
                # 生成新聞來源列表
                news_sources = [{"title": news.get("title", "No Title"), "url": news.get("url", "")} for news in daily_news]
            ...
            session.rollback()
            logger.error(f"儲存 {symbol} 的新聞資料到資料庫時發生錯誤: {e}")
        finally:
            session.close()
